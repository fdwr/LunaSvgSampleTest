...
%ifndef ALINK
%ifndef DMCLINK
%error "You must define API calls as either having a call thunk (VCLINK) or no thunk (ALINK, DMCLINK)"
%endif
%endif
%endif
...
OFN_FORCESHOWHIDDEN          equ 0x10000000    ; Show All files including System and hidden files
...
struc OPENFILENAME
.lStructSize        resd 1  ; DWORD lStructSize
.hwndOwner          resd 1  ; HWND hwndOwner
.hInstance          resd 1  ; HINSTANCE hInstance
.lpstrFilter        resd 1  ; LPCSTR lpstrFilter
.lpstrCustomFilter  resd 1  ; LPSTR lpstrCustomFilter
.nMaxCustFilter     resd 1  ; DWORD nMaxCustFilter
.nFilterIndex       resd 1  ; DWORD nFilterIndex
.lpstrFile          resd 1  ; LPSTR lpstrFile
.nMaxFile           resd 1  ; DWORD nMaxFile
.lpstrFileTitle     resd 1  ; LPSTR lpstrFileTitle
.nMaxFileTitle      resd 1  ; DWORD nMaxFileTitle
.lpstrInitialDir    resd 1  ; LPSTR lpstrInitialDir
.lpstrTitle         resd 1  ; LPCSTR lpstrTitle
.Flags              resd 1  ; DWORD Flags
.FileOffset         resw 1  ; WORD nFileOffset
.FileExtension      resw 1  ; WORD nFileExtension
.lpstrDefExt        resd 1  ; LPCSTR lpstrDefExt
.lCustData          resd 1  ; DWORD lCustData
.lpfnHook           resd 1  ; LPOFNHOOKPROC lpfnHook
.lpTemplateName     resd 1  ; LPCSTR lpTemplateName
endstruc
...
%ifdef VCLINK
  ...
  %macro api_params_already_pushed 1-2+
    ; Indirection for macro expansion concatenation with _.
    api_params_already_pushed2 _%1, %2
  %endmacro

  %macro api_params_already_pushed2 1-*
    %assign %%i (%0-1)*4
    extern %1@%%i
    call %1@%%i                 ;call indirectly via stub
  %endmacro

  %macro apijump_params_already_pushed 2
    ; Indirection for macro expansion concatenation with _.
    apijump_params_already_pushed2 _%1,%2
  %endmacro

  %macro apijump_params_already_pushed2 2
    %assign %%i (%2)*4
    extern %1@%%i
    jmp %1@%%i                  ;jmp to indirectly via stub
  %endmacro

%elifdef ALINK
; Use ALINK with custom import lib.

  %macro api_params_already_pushed 1-*
    extern %1
  ...
  %macro apijump_params_already_pushed 2
    extern %1i
    jmp [%1]                    ;jmp to indirectly through function pointer
  %endmacro

%else

  %error "Must define a linkage type, VCLINK or ALINK"
  ; Unsure which one works with DMC Linker (DMCLINK).

%endif

; Push dword parameters, and call an OS API function.
%macro api 1-2+
  pushparams32_right_to_left %2
  %if %0 > 1 ; Is there no way to just pass all parameters at once?
    api_params_already_pushed %1, %2
  %else
    api_params_already_pushed %1
  %endif
%endmacro

 ; Tail call. All the parameters are already pushed.
 ; (function name, parameter count) -> () *does not return
 %macro apijump 2
  apijump_params_already_pushed %1, %2
 %endmacro
...
; (all regs and flags saved)
WriteDebugString:
.edi equ 0 ;order of registers after using PUSHA
.esi equ 4
.ebp equ 8
.esp equ 12
.ebx equ 16
...
.ecx equ 24
.eax equ 28
.stringPtr equ 32+4+4 ; 32 bytes in saved regs, 4 in saved flags, 4 for return value
  ...
  %else
    api OutputDebugString, [esp+.stringPtr]
  %endif
    popa
    popf
    ret 4
