	       ...
	       ListGen *listgen) 
{
	...
	while (t--) 		       /* repeat TIMES times */
	{
		...
		{
		    char letter[] = "\0\0\0\0\0\0\0\0"; // FDR - for Unicode
		    char *src = e->stringval;		// not optimized at all
		    int count = e->stringlen;
		    for (; count>0; count--) {
			letter[0] = *src++;
			out (offset, segment, letter,
			     OUT_RAWDATA + wsize, NO_SEG, NO_SEG);
		    }
		    offset += e->stringlen * wsize;
		    /*
		    int align;
