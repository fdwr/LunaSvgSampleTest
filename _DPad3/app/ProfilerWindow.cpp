...
namespace
{
    ...
    UINT GetDrawTextFlags(__notnull IDWriteTextEditLayout* textLayout)
    {
        ...
        bool containsTabs = (wcschr(textLayout->GetText(), '\t') != NULL);
        ...
        textLayout->GetLineMetrics(NULL, 0, &actualLineCount);
...
void ProfilerWindow::InitializeControls()
{
    ...
    ribbonChunk = new RibbonChunk(buttonGroup, L"Select test");
        ...
        for (size_t i = 0; i < ARRAY_SIZE(g_PerfTestsTable); ++i)
        {
            ...
            StringCchPrintf(
                buffer,
                ARRAY_SIZE(buffer),
                (g_PerfTestsTable[i].function != NULL) ? L"    %s" : L"%s",
    ...
    button = new Button(this, L"Run", NULL, ControlIdRun);
    button->SetOwner(this, this);
    button->SetStyle(StyleFlagNewLine);
    button = new Button(this, L"Close", NULL, ControlIdClose);
    button->SetOwner(this, this);
}
...
void ProfilerWindow::RunTest()
{
    ...
    || textLayout_ == NULL
    ||  g_PerfTestsTable[selectedTest_].function == NULL
        )
        return;
