...
public class RuleWLife {
	...
	public int wgtAry[] = new int[10]; // weights of neighbours
	public boolean rulesS[] = new boolean[IMAXWLIFERUL + 1]; // rules for surviving
	public boolean rulesB[] = new boolean[IMAXWLIFERUL + 1]; // rules for birth
	...
	public void InitFromString(String sStr) {
		...
		while (st.hasMoreTokens()) {
			...
			if (sTok.startsWith("NW"))
				wgtAry[1] = Integer.valueOf(sTok.substring(2)).intValue();
			else if (sTok.startsWith("NN"))
				wgtAry[2] = Integer.valueOf(sTok.substring(2)).intValue();
			else if (sTok.startsWith("NE"))
				wgtAry[3] = Integer.valueOf(sTok.substring(2)).intValue();

			else if (sTok.startsWith("WW"))
				wgtAry[4] = Integer.valueOf(sTok.substring(2)).intValue();
			else if (sTok.startsWith("ME"))
				wgtAry[5] = Integer.valueOf(sTok.substring(2)).intValue();
			else if (sTok.startsWith("EE"))
				wgtAry[6] = Integer.valueOf(sTok.substring(2)).intValue();

			else if (sTok.startsWith("SW"))
				wgtAry[7] = Integer.valueOf(sTok.substring(2)).intValue();
			else if (sTok.startsWith("SS"))
				wgtAry[8] = Integer.valueOf(sTok.substring(2)).intValue();
			else if (sTok.startsWith("SE"))
				wgtAry[9] = Integer.valueOf(sTok.substring(2)).intValue();

			else if (sTok.startsWith("HI")) {
				i = Integer.valueOf(sTok.substring(2)).intValue();
				if (i >= 3) {
