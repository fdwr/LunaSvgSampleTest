...
public class MJOpen {
	...
	private Vector m_vCells; // cells found in the pattern file
	private Vector m_vDescr; // pattern description
	private Vector m_vDiv; // diversities
	...
	private void AddPattern() {
		...
		for (i = 0; i < m_vDiv.size(); i++)
			mjb.Div.ItemFromString((String) m_vDiv.elementAt(i),
		...
		for (i = 0; i < m_vCells.size(); i++) {
			cell = (CACell) m_vCells.elementAt(i);
			x = cell.x + dx;
			y = cell.y + dy;
		...
		if (bff.length() > 0) {
			...
			if ((bff.charAt(0) == '#') || (bff.charAt(0) == '!')
				...
				} else if (bff.startsWith("#D") || bff.startsWith("#C")) // description
				{
					...
					m_vDescr.addElement(sTok); // add the comment line
				...
				for (i = 0; i < bff.length(); i++) {
					if ((bff.charAt(i) >= '0') && (bff.charAt(i) <= '9')) {
						...
						if ((bff.charAt(i) == '*') || (bff.charAt(i) == 'o')
								|| (bff.charAt(i) == 'O')) {
							for (j = 0; j <= iNum - 1; j++)
								m_vCells.addElement(new CACell(
	...
	private boolean ReadMCL(Vector vLines) {
		...
		{
			bff = bff.substring(8);
			m_dMclVsn = Double.valueOf(bff).doubleValue();
		}
	...
	private boolean ProcessOneRLELine(String bff) {
		...
		if (bff.startsWith("#D") || bff.startsWith("#C")) // strange description
		{
			...
			{
				m_vDescr.addElement(bff); // add the comment line
			} else {
				if (bff.length() > 0) {
					if ((!fXYFound) && bff.startsWith("x")) // the first line
					{
						...
						while (stcomma.hasMoreTokens()) {
							...
							if (tokenType.equals("x")) // X size
								iniCol = iCol = -(Math.abs(Integer
										.parseInt(tokenValue)) / 2);
							else if (tokenType.equals("y")) // Y size
								iRow = -(Math.abs(Integer.parseInt(tokenValue)) / 2);
							else if (tokenType.equals("rule")
									|| tokenType.equals("rules"))
								m_sRules = tokenValue;
							...
							if ((bff.charAt(i) >= '0')
								...
								default: // probably a cell
									if (((bff.charAt(i) >= 'a') && (bff
										...
										for (j = 0; j <= iNum - 1; j++)
											m_vCells.addElement(new CACell(iCol
													+ j, iRow, (short) iTmp));
			...
			} else // a cell
			{
				...
				if (st.hasMoreTokens()) {
					...
					if (st.hasMoreTokens()) {
						...
						m_vCells.addElement(new CACell(iCol, iRow, (short) 1));
						fOk = true;
					}
				}
			}
