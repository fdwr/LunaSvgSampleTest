...
class MJBoard extends Panel implements Runnable {
	...
	public MJPalette mjPal; // color palette
	private boolean InitDone = false;
	private MJCellUI mjUI; // the user interface object (parent)
	...
	public int Populations[] = new int[MAX_CLO + 1]; // count of cells in each
	// state
	...
	// local controls
	private Panel pnlBotm; // bottom panel, for scrollbar and buttons
	private Scrollbar hSbar = null, vSbar = null; // scrollbars
	private int sbarWidth = 16; // scrollbars width
	private Button btnZoomIn = new Button("+");
	private Button btnZoomOut = new Button("-");
	private Button btnFit = new Button("F");

	// Universe, board
	public Point UnivSize; // universe size, can be partially invisible
	public int CellSize = 5; // size of cells
	private short crrState[][]; // the cells - current state
	private short tmpState[][]; // the cells - temporary state
	private short bakState[][]; // backup, for rewinding
	private Point ViewOrg; // view origin - left-top point
	private Point ViewSize; // actual adjusted view size, in pixels
	private Point CellsInView; // counts of visible cells in X and Y
	private Point LastPanelSize;// last panel size, for resizing handling
	...
	// misc
	public MJDiversities Div; // diversities system (black hole, nova,
	...
	public void Randomize(String sHow, int what) {
		...
		mjUI.vDescr.removeAllElements(); // no old description
		RedrawBoard(true); // show the universe
	...
	public void Seed(String sHow) {
		...
		if (sHow.startsWith("BLK")) // block of cells
		{
			...
			if (iPos >= 0) {
				int dx = Integer.valueOf(sHow.substring(0, iPos)).intValue();
				int dy = Integer.valueOf(
						sHow.substring(iPos + 1, sHow.length())).intValue();
				if (GameType == MJRules.GAMTYP_2D) {
					for (i = 0; i < dx; i++)
						for (j = 0; j < dy; j++)
