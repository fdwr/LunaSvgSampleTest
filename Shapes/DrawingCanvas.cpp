...
HRESULT DrawingCanvas::ResizeRenderTargets(SIZE size)
{
    ...
    RECT bindRect = {0,0,size.cx, size.cy};
    return targetD2D_->BindDC(target_->GetMemoryDC(), &bindRect);
}
...
{
    assert(target_ != nullptr); // should have called PaintPrepare
...
namespace
{
        ...
        )
    {
        ...
        *fileSize = 0;
        ...
        resourceHandle = FindResourceW(
            moduleHandle,
            reinterpret_cast<wchar_t const*>(resourceName),
            reinterpret_cast<wchar_t const*>(resourceType)
            );
        ...
        )
    {
        ...
        IFR(wicFactory->CreateDecoderFromFilename(
                reinterpret_cast<wchar_t const*>(fileName),
                nullptr,
                GENERIC_READ,
                WICDecodeMetadataCacheOnLoad,
                &decoder
        ...
        )
    {
        ...
        if (OpenClipboard(hwnd))
        {
            if (EmptyClipboard())
            {
                ...
                assert(destPixelsByteCount < sourceBitmapInfo.dsBmih.biSizeImage);
