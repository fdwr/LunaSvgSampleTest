...
enum DWRITE_LINE_GAP_MODE
{
    // Add specified constant to line height (not using the font's line gap).
    DWRITE_LINE_GAP_MODE_ADDITIVE,

    // Multiply by font's line gap. Using 0.5 would match CSS.
    DWRITE_LINE_GAP_MODE_MULTIPLE
};


enum DWRITE_JUSTIFICATION_MODE
{
    DWRITE_JUSTIFICATION_MODE_NONE,
    DWRITE_JUSTIFICATION_MODE_DEFAULT
};


// RFC: probably remove font scale mode, as the application should be able to accomplish this in the callback.
enum DWRITE_FONT_SCALE_MODE
{
    // Font size scales the em box. This is the default, but it leads to
    // ragged appearance of differing fonts adjacent to each other.
    DWRITE_FONT_SCALE_MODE_EM,

    // Font size is the height of a lowercase letter (normalization used in CSS3).
    DWRITE_FONT_SCALE_MODE_X_HEIGHT,

    // Font size is the height of a capital letter (or the H-height).
    DWRITE_FONT_SCALE_MODE_CAP_HEIGHT,

    // Font size is the text height, sum of ascent + descent.
    DWRITE_FONT_SCALE_MODE_TEXT_HEIGHT,
};


// RFC: ** These might be accomplished with baseline shift/rise alone, making these unnecessary. Though, I still think that even if they can accomplish superscript/subscript/hanging and the like, that we may need it for top/bottom alignment, and it would make vertical text easier too, to just center around the central baseline. **

enum DWRITE_BASELINE_ALIGNMENT
{
    // Align to Latin/Greek/Cyrillic baseline at bottom of letters.
    DWRITE_BASELINE_ALIGNMENT_ALPHABETIC,

    // Align to central baseline, halfway between ascent and descent.
    DWRITE_BASELINE_ALIGNMENT_CENTRAL,

    // Align to top/right.
    DWRITE_BASELINE_ALIGNMENT_TOP,
    DWRITE_BASELINE_ALIGNMENT_RIGHT = DWRITE_BASELINE_ALIGNMENT_TOP,

    // Align to bottom of text.
    DWRITE_BASELINE_ALIGNMENT_BOTTOM,
    DWRITE_BASELINE_ALIGNMENT_LEFT = DWRITE_BASELINE_ALIGNMENT_BOTTOM,

    // Align to ideographic baseline, used in CJK.
    DWRITE_BASELINE_ALIGNMENT_IDEOGRAPHIC,

    // Align to hanging baseline, used in scripts like Devanagari.
    DWRITE_BASELINE_ALIGNMENT_HANGING,

    // Align to math baseline, used in math equations.
    DWRITE_BASELINE_ALIGNMENT_MATH,

    // Align to middle baseline.
    DWRITE_BASELINE_ALIGNMENT_MINISCULE_MIDDLE,

    // Align to height of majuscule letter (H-height).
    DWRITE_BASELINE_ALIGNMENT_CAP_HEIGHT,

    // Align to height of miniscule letter (x-height).
    DWRITE_BASELINE_ALIGNMENT_X_HEIGHT,

    // Align to ascent.
    DWRITE_BASELINE_ALIGNMENT_ASCENT,

    // Align to descent.
    DWRITE_BASELINE_ALIGNMENT_DESCENT,

    // Align to superscript.
    DWRITE_BASELINE_ALIGNMENT_SUPER,

    // Align to subscript.
    DWRITE_BASELINE_ALIGNMENT_SUB,
};
...
// The common D2D/DirectImage/DWrite rect
struct DX_RECT_F
{
    FLOAT left;
    FLOAT top;
...
typedef DX_RECT_F DWRITE_RECT;

// Too bad DWRITE_TEXT_RANGE hadn't been just a tad more generic.
// Then we could just have a DWRITE_RANGE and inherit or typedef from it
// so that people could easily write little helpers/adapters to simplify
// its use, like found in DevTest LayoutTest.
struct DWRITE_CHARACTER_RANGE
{
   UINT32 first;
   UINT32 length;
};
...
// Single kerning pair. Kerning amount is in font design units.
// * should it be in other units?
// * should we return glyph pairs like the 'kern' table does,
//   or character codes? GDI returns 16bit character codes,
//   which is limited to the lower BMP. The kern table itself
//   actually stores glyph ids.
//
struct DWRITE_KERNING_PAIR
{
   UINT32 firstGlyph;
   UINT32 secondGlyph;
   UINT32 adjustment;
...
/// Various font data such as metrics, names and glyph outlines is obtained from IDWriteFontFace.
/// </summary>
interface DWRITE_DECLARE_INTERFACE("5f49804d-7024-4d43-bfa9-d25984f53850") IDWrite2FontFace : public IDWriteFontFace
{
    /// <summary>
    /// Get the specific glyph ID of the given base character and selector.
    /// This is useful for CJK characters and math symbols.
    /// </summary>
    /// <param name="baseCharacter">Base Unicode character.</param>
    /// <param name="selector">Variation selector, including the ranges
    ///     FE00..FE0F (VS1..VS16), E0100..E01EF (VS17..VS256), and
    ///     180B..180D (FVS1..FVS3). Passing 0 is a no-op.</param>
    /// <returns>
    /// The glyph index of the pair, nominal glyph index if that selector has
    /// no alternate glyph, or 0 if the base character has no mapping.
    /// </returns>
    STDMETHOD_(UINT16, GetGlyph)(
        UINT32 baseChar,
        UINT32 selector
        ) PURE;

    // Each of these forms have their place. Though, we don't *truly* need
    // both of them, since you can get from one to the other.
    //
    // The first one has the advantage that it can tell you how many ranges
    // exist up front, in those cases where you are caching the ranges.
    // The second one has the benefit that you need not allocate any
    // temporary memory if you are just enumerating through the glyphs.
    // If we were to keep only one, I'd keep the array one, since it is more
    // similar to the existing GDI function for migrating apps, and since it
    // can tell you the count up front.

    // Returns the Unicode ranges, similar to GDI's GetFontUnicodeRanges
    // (except that it works with FULL Unicode, not just 16-bit).
    // This retrieves the entire CMAP.
    STDMETHOD(GetCharacterRanges)(
        UINT32 maxRangeCount,
        __out_ecount_opt(maxRangeCount) DWRITE_CHARACTER_RANGE* characterRanges,
        __out UINT32* actualRangeCount
        ) PURE;

    // Given a codepoint, this returns the next codepoint supported in the font.
    // This is more useful when you don't want to allocate a large buffer,
    // and this exact same function is used internally for our fast path in
    // FontCmap::FindNextCodepoint().
    STDMETHOD(GetNextCodepointRange)(
        UINT32 startingCodepoint,
        __out DWRITE_CHARACTER_RANGE* range
        ) PURE;
    ...
    /// <summary>
    /// Returns TRUE if the font contains vertical OpenType features
    /// (vert, vrt2, vkrn...). This can be used in determining whether or
    /// not you want to apply vertical forms. The recommendation is that
    /// you orient characters according to their own properties and the
    /// preferred orientation, agnostic to whether the font supports it
    /// or not; but this may be useful for backwards compatibility in apps
    /// that want to rotate all characters if the font does not contain
    /// vertical features.
    /// </summary>
    STDMETHOD_(BOOL, HasVerticalFeatures)() PURE;

    // Retrieve the nominal glyph ids, optionally retrieve the vertical
    // forms.
    STDMETHOD(GetGlyphIndices)(
        __in_ecount(codePointCount) UINT32 const* codePoints,
        UINT32 codePointCount,
        BOOL useVerticalForms,
        __out_ecount(codePointCount) UINT16* glyphIndices
        ) PURE;

    // Directly get the glyph advances, for times when you really
    // don't need all the other metrics which you would just allocate
    // a temporary buffer for and throw away anyway.
    // ** do we care about favoring GDI-like modes completely? **
    // ** honestly, all the extra pixelsPerDip and matrix stuff **
    // ** unnecessarily complicates it for almost no value **
    STDMETHOD(GetGlyphAdvances)(
        __in_ecount(glyphCount) UINT16 const* glyphIndices,
        UINT32 glyphCount,
        DWRITE_MEASURING_MODE measuringMode,
        FLOAT emSize,
    ...
    STDMETHOD(GetDesignGlyphAdvances)(
        ...
        UINT32 glyphCount,
        BOOL isSideways,
        __out_ecount(glyphCount) INT32* glyphAdvances
        ) PURE;

// todo: do we also need a more generic DoesThisSpecificGlyphHaveAVerticalForm function?

    // todo: Add three font feature enumeration functions.

    // todo: Add glyph applicability function

    // todo: Add function to apply the kerning.
};
...
/// The IDWriteFont interface represents a physical font in a font collection.
/// </summary>
interface DWRITE_DECLARE_INTERFACE("acd16696-8c14-4f5d-877e-fe3fc1d32738") IDWrite2Font : public IDWriteFont
{
    /// <summary>
    /// Returns TRUE if the font is monospaced.
    /// </summary>
...
enum DWRITE_READING_DIRECTION
{
    ...
    /// <summary>
    /// Reading direction has negative coordinate direction.
    /// </summary>
    DWRITE_READING_DIRECTION_REVERSE    = 0x01,

    /// <summary>
    /// Reading direction is horizontal, either LTR or RTL
    /// depending on whether reversed.
    /// </summary>
    DWRITE_READING_DIRECTION_HORIZONTAL= 0x00, 

    /// <summary>
    /// Reading direction is vertical, either TTB or BTT
    /// depending on whether reversed.
    /// </summary>
    DWRITE_READING_DIRECTION_VERTICAL   = 0x02,
};


/// <summary>
/// Direction for how lines of text progress relative to one another.
/// There are only two flow directions, not four. Otherwise there would
/// be a conflict between reading direction and flow direction, where
/// you could set reading direction left-to-right and flow direction
/// right-to-left. This situation should never be allowed, so the secondary
/// direction (line stacking) is always relative to and perpendicular to the
/// reading direction.
/// </summary>
enum DWRITE_FLOW_DIRECTION
{
    /// <summary>
    /// Text lines increase perpendicular to the reading direction.
    /// </summary>
    DWRITE_FLOW_DIRECTION_INCREASING,

    /// <summary>
    /// Text lines decrease perpendicular to the reading direction.
    /// </summary>
    DWRITE_FLOW_DIRECTION_DECREASING,

    /// <summary>
    /// Lines progress top to bottom.
    /// </summary>
    DWRITE_FLOW_DIRECTION_TOP_TO_BOTTOM = DWRITE_FLOW_DIRECTION_INCREASING,

    /// <summary>
    /// Lines progress bottom to top.
    /// </summary>
    DWRITE_FLOW_DIRECTION_BOTTOM_TO_TOP = DWRITE_FLOW_DIRECTION_DECREASING,

    /// <summary>
    /// Lines progress left to right (traditional vertical Mongolian).
    /// </summary>
    DWRITE_FLOW_DIRECTION_LEFT_TO_RIGHT = DWRITE_FLOW_DIRECTION_INCREASING,

    /// <summary>
    /// Lines progress left to right (traditional vertical Japanese/Chinese).
    /// </summary>
    DWRITE_FLOW_DIRECTION_RIGHT_TO_LEFT = DWRITE_FLOW_DIRECTION_DECREASING,
};


enum DWRITE_GLYPH_ORIENTATION
{
    // Masks
    DWRITE_GLYPH_ORIENTATION_FLIP_H     = 0x01, // Glyph mirrored horizontally  (left <-> right)
    DWRITE_GLYPH_ORIENTATION_FLIP_V     = 0x02, // Glyph mirrored vertically (top <-> bottom)
    DWRITE_GLYPH_ORIENTATION_UPRIGHT    = 0x04, // Glyph has upright default orientation
    DWRITE_GLYPH_ORIENTATION_SIDEWAYS   = 0x08, // Glyph rotated counterclockwise / sideways (270)
    DWRITE_GLYPH_ORIENTATION_FORCED     = 0x10, // Force orientation, even if unusual

    /// <summary>
    /// Enum aliases:
    /// Clockwise is most common for CJK text.
    /// Counterclockwise can be found on book spines and ledgers.
    /// </summary>
    DWRITE_GLYPH_ORIENTATION_UP         = 0,
    DWRITE_GLYPH_ORIENTATION_CCW        = DWRITE_GLYPH_ORIENTATION_SIDEWAYS,
    DWRITE_GLYPH_ORIENTATION_CW         = DWRITE_GLYPH_ORIENTATION_SIDEWAYS
                                        | DWRITE_GLYPH_ORIENTATION_FLIP_H
                                        | DWRITE_GLYPH_ORIENTATION_FLIP_V,
};
...
/// <summary>
/// Line wrapping in multiline paragraph.
/// </summary>
enum DWRITE_WRAPPING_MODE
{
    /// <summary>
    /// Words are broken across lines to avoid text overflowing the layout box.
    /// If the layout is thinner than a single word, the word is broken.
    /// </summary>
    DWRITE_WRAPPING_MODE_WORD,

    /// <summary>
    /// Words are kept within the same line even when it overflows the layout box.
    /// This option is often used with scrolling to reveal overflow text. 
    /// </summary>
    DWRITE_WRAPPING_MODE_NONE,

    /// <summary>
    /// If layout is thinner than a single word, the flows beyond the edge rather
    /// than being split.
    /// </summary>
    DWRITE_WRAPPING_MODE_WHOLE_WORD,

    /// <summary>
    /// Wrap between any characters, even in the middle of words.
    /// This is used in Far East text.
    /// </summary>
    //DWRITE_WORD_WRAPPING_WRAP_ANYWHERE,

    DWRITE_WORD_WRAPPING_WRAP = DWRITE_WRAPPING_MODE_WORD,
    DWRITE_WORD_WRAPPING_NO_WRAP = DWRITE_WRAPPING_MODE_NONE,
};

typedef DWRITE_WRAPPING_MODE DWRITE_WORD_WRAPPING;
...
struct DWRITE2_TRIMMING : public DWRITE_TRIMMING
{
    // Whether to keep the text of the front (trimming the back)
    // middle (trimming inside out), or back (trimming the front).
...
enum DWRITE_LINE_END_TYPE
{
    // TLF has TextLineCreationResult = complete, emergency, insufficientWidth,  success
    //      http://livedocs.adobe.com/labs/textlayout/flash/text/engine/TextLineCreationResult.html
    // 

    // Text was wrapped normally (at available breakpoint)
    DWRITE_LINE_END_TYPE_WRAP,

    // Text was wrapped in the middle of a word (emergency breaking because line too thin)
    // WPF's TextLine::IsTruncated property.
    DWRITE_LINE_END_TYPE_SPLIT,

    // Object had must-break specified
    DWRITE_LINE_END_TYPE_OBJECT,

    // Encountered end of paragraph.
    DWRITE_LINE_END_TYPE_END_OF_PARAGRAPH,

    // Reached end of text.
    DWRITE_LINE_END_TYPE_END_OF_TEXT,

    // Hit text length limit.
    DWRITE_LINE_END_TYPE_TEXT_LENGTH_LIMIT,

    // Hyphenation occurred. *this may not be exposed in v2
    DWRITE_LINE_END_TYPE_HYPHENATION,
};


// These allow selective rendering of only certain elements.
// This can be used to reduce state switching while drawing
// or to control the painting order.
//
// rfc: Should we have a fixed set of recognized decorations, or, should we just
//      allow the client to query the line efficiently such that it is able to
//      easily accomplish ANY decoration?
//
enum DWRITE2_TEXT_RENDERER_ELEMENT
{
    DWRITE2_TEXT_RENDERER_ELEMENT_TEXT          = 0x00000001,
    DWRITE2_TEXT_RENDERER_ELEMENT_UNDERLINE     = 0x00000002,
    DWRITE2_TEXT_RENDERER_ELEMENT_STRIKETHROUGH = 0x00000004,
    DWRITE2_TEXT_RENDERER_ELEMENT_OVERLINE      = 0x00000008,
    DWRITE2_TEXT_RENDERER_ELEMENT_BORDER        = 0x00000010,
    DWRITE2_TEXT_RENDERER_ELEMENT_EMPHASIS      = 0x00000020,

    DWRITE2_TEXT_RENDERER_ELEMENT_ALL = DWRITE2_TEXT_RENDERER_ELEMENT_TEXT
                                      | DWRITE2_TEXT_RENDERER_ELEMENT_UNDERLINE
                                      | DWRITE2_TEXT_RENDERER_ELEMENT_STRIKETHROUGH
                                      | DWRITE2_TEXT_RENDERER_ELEMENT_OVERLINE
                                      | DWRITE2_TEXT_RENDERER_ELEMENT_BORDER
                                      | DWRITE2_TEXT_RENDERER_ELEMENT_EMPHASIS,
};

#ifdef DEFINE_ENUM_FLAG_OPERATORS
DEFINE_ENUM_FLAG_OPERATORS(DWRITE2_TEXT_RENDERER_ELEMENT);
#endif



enum DWRITE_RUN_FORMAT_TYPE
{
    // Null range (such as buffer gap or hidden text)
    DWRITE_RUN_FORMAT_TYPE_NULL,

    // Run of pure characters.
    DWRITE_RUN_FORMAT_TYPE_TEXT,

    // Inline object.
    DWRITE_RUN_FORMAT_TYPE_OBJECT,

    // Explicit end of line (equivalent to line separator).
    DWRITE_RUN_FORMAT_TYPE_END_OF_LINE,

    // Explicit paragraph end (equivalent to paragraph separator).
    DWRITE_RUN_FORMAT_TYPE_END_OF_PARAGRAPH,

    // End of text.
    DWRITE_RUN_FORMAT_TYPE_END_OF_TEXT,
};


/// <summary>
/// Flags which features are passed to the line formatter in the DWRITE_RUN_FORMAT.
/// * Note, all unknown bits must be zero. Just as a check, the greatest bit must
///   explicitly be 0, and the second greatest must be 1. If they are not, this is
///   an error.
/// </summary>
enum DWRITE_RUN_FORMAT_MASK
{
    // These will likely evolve...
    DWRITE_RUN_FORMAT_MASK_READING_DIRECTION    = 0x00000001,
    DWRITE_RUN_FORMAT_MASK_ALIGNMENT            = 0x00000002,
    DWRITE_RUN_FORMAT_MASK_TEXT_DECORATIONS     = 0x00000004,
    DWRITE_RUN_FORMAT_MASK_WRAPPING             = 0x00000008,
    DWRITE_RUN_FORMAT_MASK_LINE_SPACING         = 0x00000010,
    DWRITE_RUN_FORMAT_MASK_MEASURING_MODE       = 0x00000020,
    DWRITE_RUN_FORMAT_MASK_LETTER_SPACING       = 0x00000040,
    DWRITE_RUN_FORMAT_MASK_BASELINE             = 0x00000080,
    DWRITE_RUN_FORMAT_MASK_FONT                 = 0x00000100,
    DWRITE_RUN_FORMAT_MASK_LOCALE               = 0x00000200,
    DWRITE_RUN_FORMAT_MASK_TYPOGRAPHY           = 0x00000400,
    DWRITE_RUN_FORMAT_MASK_NUMBER_SUBSTITUTION  = 0x00000800,
    DWRITE_RUN_FORMAT_MASK_DRAWING_EFFECT       = 0x00001000,
    DWRITE_RUN_FORMAT_MASK_INLINE_OBJECT        = 0x00002000,
    DWRITE_RUN_FORMAT_MASK_PADDING              = 0x00004000,

    // Force at least 32bit
    DWRITE_RUN_FORMAT_FORCE_ENUM_SIZE = 0xFFFFFFFF
};


/// <summary>
/// Formatting information returned from the application.
/// </summary>
/// <remarks>
/// This is returned by the application in the callback to
/// IDWriteRunFormatSource::GetRunFormat.
///
/// The very same structure can be reused each call. This lets you fill
/// all of them in to defaults, only modifying those which actually vary
/// between calls. For example, if the wrapping mode and text decorations
/// are constant in your backing store, and only the font size varies,
/// you can just set the other attributes once, then vary just the size
/// for each run fetch call.
///
/// All contained pointers are weak and owned by the application.
/// If DWrite needs one of them, it will obtain an additional reference
/// or copy them out. The callee should not pre-increment the ref-count.
/// Unlike actual COM objects via out parameters, this data structure is not
/// an object created on the caller's behalf and later taken over by the
/// or freed by the caller. It is merely a constant read-only glimpse into
/// the application's formatting information. Similar to the existing
/// IDWriteTextAnalysis::GetText and IDWriteTextAnalysis::GetLocaleName,
/// the out parameter is constant and non-freeable. This greatly reduces
/// potential to screw up on both sides regarding when fields need to be
/// pre-incremented and post-decremented depending on what is being asked
/// for and what is being ignored.
///
/// All fields must remain valid until the next run retrieval call (or
/// until the end of the line layout process), like the existing
/// IDWriteTextAnalysis::GetText.
///
/// The application should return all the attributes requested, but if it
/// does not, the layout process will not fail. Instead, it will simply
/// use default attributes. This allows forward feature addition in future
/// versions without the application worrying about what to do about requests
/// for unrecognized attributes.
///
/// The application is allowed to return more attributes than are actually
/// asked for without harm. Though, they may be ignored. Also, the same
/// run and offset may be queried more than once.
///
/// Baseline offset does not change the line height and can be used to
/// implement superscripts that do not create a gap between lines, or to
/// implement the lowered E in the TEX logo. The x offset is useful for
/// superscripts in italic fonts that would otherwise have an odd gap
/// (using the OS/2 ySuperscriptXOffset field) or to ensure fine placement
/// of decorations, drop caps, or optically realigned glyphs at the start
/// of a line.
/// </remarks>
struct DWRITE_RUN_FORMAT
{
    // Which attributes are actually present, set upon returning from app.
    // This variable MUST be set as output. All others are optional, yielding
    // defaults if not indicated as present.
    DWRITE_RUN_FORMAT_MASK presentAttributes;

    // Reading direction
    DWRITE_READING_DIRECTION readingDirection;  // explicit direction, derived from font binding/IME language

    // Alignment
    DWRITE_TEXT_ALIGNMENT textAlignment;

    // Wrapping
    DWRITE_WORD_WRAPPING wrappingMode;

    // Line spacing.
    DWRITE_LINE_GAP_MODE lineGapMode;           // whether to use font's line gap or custom line gap
    FLOAT lineGapAbove;                         // constant added above line, or proportion of font's line gap
    FLOAT lineGapBelow;                         // constant added below line, or proportion of font's line gap
    UINT32 invisibleCharactersAffectHeight : 1; // consider the height of formatting and line breaking
                                                // characters when determining line height, like v1 behavior.
                                                // Otherwise only visible characters affect height, except the
                                                // case of an entirely empty line, which still uses the height
                                                // of the invisible line separators (CR/LF/LS/PS).

    // Letter spacing
    FLOAT letterSpacingBefore;                  // tracking added before clusters
    FLOAT letterSpacingAfter;                   // tracking added after clusters

    // Baseline
    DWRITE_BASELINE_ALIGNMENT baselineAlignment;
    FLOAT baselineShift;                        // vertical shift, useful for superscripts/subscripts
    FLOAT baselineOffsetX;                      // horizontal offset, useful for fine placement
    FLOAT baselineOffsetY;                      // vertical offset, fine placement does not change line height

    // Measuring mode
    DWRITE_MEASURING_MODE measuringMode;

    // Font selection
    IDWriteFontFace* fontFace;                  // explicit font face already known (NULL for normal font fallback)
    IDWriteFontCollection* fontCollection;      // font collection to look in for families
    WCHAR const* fontFamilyName;                // family name
    FLOAT fontSize;                             // font size
    // RFC: DWRITE_FONT_SCALE_MODE fontScaleMode; // interpret size as em, x-height, cell height...
    DWRITE_FONT_WEIGHT fontWeight;
    DWRITE_FONT_STYLE fontStyle;
    DWRITE_FONT_STRETCH fontStretch;

    // Locale
    WCHAR const* localeName;                    // locale of run (such as 'en-US', 'ar-EG', 'ja-JP')

    // Text decorations
    // rfc: Should we have a fixed set of recognized decorations, or, should we just
    //      allow the client to query the line efficiently such that it is able to
    //      easily accomplish ANY decoration?
    UINT32 hasUnderline         : 1;            // this run is underlined
    UINT32 hasOverline          : 1;            // this run is overlined
    UINT32 hasStrikethrough     : 1;            // this run has a strikethrough
    UINT32 hasBorder            : 1;            // this run has a border
    UINT32 hasEmphasisMark      : 1;            // emphasis marks exist over text

    // Padding/borders/margins are all factored into this
    FLOAT paddingLeft;                          // size of padding at left edge   (regardless of text direction)
    FLOAT paddingTop;                           // size of padding at top edge    (regardless of text direction)
    FLOAT paddingRight;                         // size of padding at right edge  (regardless of text direction)
    FLOAT paddingBottom;                        // size of padding at bottom edge (regardless of text direction)
    UINT32 paddingBegins        : 1;            // this run starts padding
    UINT32 paddingEnds          : 1;            // this run ends padding
    UINT32 paddingBreaksOnLine  : 1;            // should break at the line split (like Word, not CSS)
    ...
    // * Note that color, drawing effects, and meta-data are not part of this
    //   structure. Instead, the application uses the corresponding runId.
};


/// <summary>
/// Application supplied source for line layout formatting.
/// </summary>
interface DWRITE_DECLARE_INTERFACE("9c906818-31d7-4fd3-a151-7c5e225db55a") IDWriteRunFormatSource : public IUnknown
{
    STDMETHOD(GetRunType)(
        UINT_PTR runId,
        UINT32 textOffset,
        __out DWRITE_RUN_FORMAT_TYPE* type,
        __out UINT32* runLength
        ) PURE;

    // Get a raw pointer to the text in the client's backing store.
    // This pointer must remain valid while still in the layout process.
    STDMETHOD(GetRunText)(
        UINT_PTR runId,
        UINT32 textOffset,
        __out WCHAR const** text,
        __out UINT32* runLength
        ) PURE;

    // Returns a pointer to the formatting information of the given run and offset.
    // This formatting info remains valid until the next call. If the line layout
    // needs to keep any of the objects, it will take a reference or make a copy.
    // The application should not add an additional reference itself.
    STDMETHOD(GetRunFormat)(
        UINT_PTR runId,
        UINT32 textOffset,
        DWRITE_RUN_FORMAT_MASK requestedAttributes,
        __deref_out DWRITE_RUN_FORMAT const** runFormat,
        __out UINT32* runLength
        ) PURE;

    // Due to non-contiguous backing stores (fragmented or with buffer gaps)
    // the next text offset could point several counts later or be reset
    // to a lower offset, since the offsets are relative to each run.
    STDMETHOD(Advance)(
        UINT32 textLength,
        __inout UINT_PTR* runId,
        __inout UINT32* textOffsetRelativeToRun
        ) PURE;
    ...
    // RFC: It's been suggested that both Advance and Recede be merged
    //      into a single signed-parameter function, such as MoveBy.
    //      This should be fine, since we should not have single runs
    //      longer than 2GB's (the line itself would run out of
    //      physical memory).
};
...
/// This object may not be thread-safe and it may carry the state of text format change.
/// </remarks>
interface DWRITE_DECLARE_INTERFACE("9c906818-31d7-4fd3-a151-7c5e225db55b") IDWrite2TextFormat : public IUnknown
{
    /// <summary>
    /// Sets the orientation of glyphs.
    /// </summary>
    /// <param name="readingDirection">Text reading direction</param>
    /// <returns>
    /// Standard HRESULT error code.
    ...
    STDMETHOD(SetGlyphOrientation)(
        DWRITE_GLYPH_ORIENTATION glyphOrientation
        ) PURE;
};
...
/// <summary>
/// These hold script-specific properties. For locale properties (related to
/// language and country) use NLS instead.
///
/// Refer to Uniscribe SCRIPT_PROPERTIES Structure for existing precedent:
/// http://msdn.microsoft.com/en-us/library/dd374042(VS.85).aspx
///
/// Check ICU's properties:
/// http://userguide.icu-project.org/strings/properties
/// </summary>
struct DWRITE_SCRIPT_PROPERTIES
{
    /// <summary>
    /// The four letter codes are standardized and found on the Unicode
    /// website. The function returns both the letter code and number 0-999.
    /// ISO 15924 actual has more scripts for bibliographic distinction than
    /// Unicode considers unique. In these rare cases, the default script will
    /// be returned. For example, Latin will return Latn, not Latg for the
    /// Gaelic Latin variant.
    ///
    ...
    /// <summary>
    /// The numeric code ranges 0-999.
    ///
    /// http://unicode.org/iso15924/iso15924-codes.html
    /// </summary>
    UINT32 isoScriptNumber;

    // Many scripts allow more than one direction, but most have a default
    // historic direction. todo: should historic be distinguished from modern?
    // For example, the traditional default for Japanese was ttb/rtl, but the
    // modern default in text books is ltr/ttb?
    // Unknown/common scripts default to LTR, consistently with the Unicode
    // bidi file.
    DWRITE_READING_DIRECTION defaultReadingDirection;
    DWRITE_FLOW_DIRECTION    defaultFlowDirection;

    // rfc: horizontal vs vertical defaults?
    // rfc: modern vs historic?

    // Number of characters to estimate needed look-ahead for complex languages.
    // Latin and all Kana are generally one. Indic scripts are 15, and others
    // are 8. Note that combining marks and variation selectors can produce
    // clusters longer than these look-aheads, which must be tested explicitly.
    // These are typical language use.
    //
    // Diacritic abuse:
    //  http://blogs.msdn.com/michkap/archive/2006/02/17/533929.aspx
    //
    // This was added for the WPF DCR, so that they can use our table rather
    // than keep their own. This is equivalent to their maximumClusterSize.
    //
    // rfc: is this a sufficient replacement for Uniscribe fClusterSizeVaries?
    ...
    // - Has combining and reordering of characters.
    // - Has specialized word-breaking and justification rules.
    // - Filters out illegal character combinations.

    // This script requires shaping, such as Arabic and Devanagari.
    // Latin and CJK only need shaping when diacritics are present,
    // such as in Vietnamese and decomposed Japanese voicing marks.
    // Usually, European and CJK languages use precomposed forms.
    // todo: What about Hangul? It has both precomposed and decomposed,
    //       which means we would need to set true.
    // fComplex
    UINT32 requiresShaping              : 1;

    // Restrict the caret to whole clusters, like Thai and Indian.
    // Some like Arabic allow navigation between clusters.
    //
    // SCRIPT_PROPERTIES::fNeedsCaretInfo
    // http://msdn.microsoft.com/en-us/library/dd374042(VS.85).aspx
    //
    // We'll just use Uniscribe and WPF's existing data here.
    UINT32 restrictCaretToGlyphClusters : 1;

    // This script is Chinese/Japanese/Korean, and uses block justification.
    // It also naturally aligns to a grid, such as Han, Hiragana, Katakana,
    // Hangul, Bopomofo, and Yi. Other scripts center words/sentences within the
    // grid, but do not align individual characters.
    UINT32 isBlockForm                  : 1;

    // Whether the language is one like Latin that uses word dividers,
    // unlike Thai or Chinese, which instead prioritize inter-character
    // distribution over inter-word.
    //
    // rfc: Most languages uses the space as a divider, but modern Ethiopic
    //      uses the double interpunct that looks like a colon (though space
    //      usage is increasing). How should this case be handled? What does
    //      Ethiopic justification look like? In the case of no spaces, should
    //      the caller just intercharacter? I think it's fine given that we
    //      also have the justification character.
    //
    // Yes: Latin, Arabic, Ethiopic
    // No:  Thai, Chinese, Japanese
    //
    // Note that Japanese does use the interpunct for transcribed foreign
    // words, but this is not normal use.
    // http://en.wikipedia.org/wiki/Interpunct
    UINT32 usesWordDividers             : 1;

    // Words can be broken in the middle of lines.
    //
    // Yes: Chinese, Japanese (except in certain kinsoku breaking rule cases),
    //      Korean (uses spaces, but can allow word breaking)
    // No:  Arabic, Thai, Latin (except if hyphenated).
    //
    // http://office/14/howto/Internationalization/Wiki%20Pages/Korean%20Word%20Breaking.aspx
    UINT32 allowsInterwordBreaks        : 1;

    // Script is cursive by default, such as Arabic.
    // This is useful to determine whether justification should be applied
    // using kashida extension. Otherwise, the app could just use simpler
    // spacing increase, between words or letters.
    //
    // Yes: Arabic, Syriac
    // No:  Thaana, Devanagari, Chinese, Latin, Hebrew
    //
    // Latin and Japanese also supports cursive forms, but they are not so
    // by default. Hebrew has handwritten form, but the letters are not
    // connected, so the term does not apply.
    //
    UINT32 isCursive                    : 1;

    // Natural cursive scripts and baseline linked scripts like Devanagari.
    // Any cursive scripts are connected, but not all connected scripts are cursive.
    //
    // Yes: Devanagari, Arabic, Syriac
    // No:  Chinese, Thaana, Latin
    //
    // rfc: Should connected and cursive be merged?
    //
    UINT32 isConnected                  : 1;

    // This script should use isolated forms when coerced to sideways.
    //
    // An example of this is Arabic in Excel text cells.
    // Other scripts just shape as usual and then turn each
    // cluster sideways.
    //
    UINT32 useIsolatedFormWhenSideways  : 1;

    UINT32 padding                      : 32 - 10;

    /// <summary>
    /// An appropriate character/glyph to elongate the given script
    /// for justification. You can get glyph glyph for this character
    /// and pass it to InsertJustificationGlyphs(), when it uses
    /// something other than the simple space.
    ///
    /// Arabic      -> U+0640 U+0640 ARABIC TATWEEL
    /// Mongolian   -> U+180A MONGOLIAN NIRUGU
    /// N'Ko        -> U+07FA NKO LAJANYALAN
    /// Latin       -> U+0020 SPACE
    /// </summary>
...
// Undecided properties.
    ...
    // This is informative, as DWrite itself does not hyphenate.
    // Applications must include hyphenation dictionaries.
    // Yes: Latin
    // No:  Arabic, Chinese
    //
    // rfc: Expose this? since we don't actually do any hyphenation
    //      I'd understand reluctance to say whether or not it even
    //      applies.
    // UINT32 allowsHyphenation            : 1;

    // rfc: fNeedsWordBreaking
    //          Easy to expose.

    // rfc: bCharSet? - should we expose this? I don't think so.
    //                  there are many scripts that simply have no
    //                  corresponding Windows character set. I think
    //                  these should be deprecated, once and for all.
    //
    //                  ANSI_CHARSET
    //                  BALTIC_CHARSET
    //                  CHINESEBIG5_CHARSET
    ...
    // rfc: fClusterSizeVaries?
    //          I'm unsure the exact scenario of this, but I could see it being useful
    //          during line layout to estimate whether the script needs more lookahead.

    // rfc: fRejectInvalid - ask Chai
    //          It would be easy to add, but where is this used?
    //          I see use in lpk_edit.c EditVerifyText by Chai 1997.

    // rfc: fControl
    //      Value indicating if only control characters are used in the script.
    //      Isn't the current shapes flag enum hidden sufficient?

////////////////////////////////////////
// Removable properties. Maybe useful somewhere,
// but less useful overall. Will not be exposed.

    /// <summary>
    /// The character used for division between words.
    ///
    /// Latin = space
    /// Runic = U+16EB and U+16EC
    /// Ethiopic = U+1361 ETHIOPIC WORDSPACE
    /// Ugaritic = U+12470 CUNEIFORM PUNCTUATION SIGN OLD ASSYRIAN WORD DIVIDER
    ///
    /// http://en.wikipedia.org/wiki/Interpunct
    /// http://en.wikipedia.org/wiki/Word_divider
    /// </summary>
    // UINT32 wordDividerCharacter;

    /// <summary>
    /// A single strong exemplar character of the given script.
    /// Where possible, these correspond to the last resort glyphs shown
    /// by Unicode.
    ///
    /// http://unicode.org/charts/lastresort.html
    ///
    /// Latin    -> U+004C LATIN CAPITAL LETTER L
    /// Greek    -> U+03A9 GREEK CAPITAL LETTER OMEGA
    /// Hebrew   -> U+05D0 HEBREW LETTER ALEF
    /// Thai     -> U+0E17 THAI CHARACTER THO THAHAN
    /// Hiragana -> U+304B HIRAGANA LETTER KA
    /// </summary>
    // UINT32 exemplarCharacter;
};
...
{
    /// <summary>
    /// Minimum allowed width in DIPs of this glyph. Zero is allowed.
    /// Negative numbers are not valid.
    /// </summary>
    FLOAT minimumExpansion;

    /// <summary>
    /// Maximum allowed width in DIPs of this glyph. Positive infinity is allowed.
    /// </summary>
    FLOAT maximumExpansion;

    /// <summary>
    /// Glyphs with lower values are expanded first.
    /// Only after all the glyphs in a given level are processed will the
    /// next level be processed.
    /// </summary>
    UINT32 justificationLevel       : 8;

    // ** Note, if we support a discreteWidthcount and discreteWidths list,
    //          then we should also expose the glyph extender list or glyph
    //          variants at the font level
    // **

    /// <summary>
    /// Number of discrete widths available. If zero, any continuous width is
    /// allowed/ between min and max. If greater than zero, discreteWidths must
    /// be non-null. This is useful for fonts that support multiple forms of a
    /// glyph at varying sizes.
    /// </summary>
    UINT8 discreteWidthCount        : 8;

    /// <summary>
    /// This glyph can be used during residual expansion after all other
    /// options are exhausted.
    /// </summary>
    UINT8 allowResidualExpansion    : 1;

    /// Must be zero.
    UINT32 padding                  : 32 - (8+8+1);

    /// <summary>
    /// Optional pointer to a list of advance widths in DIPs,
    /// arranged in ascending order. This only applies if the widths count
    /// is greater than zero.
    /// </summary>
    __field_ecount_opt(discreteWidthCount) FLOAT* discreteWidths;
};
...
interface DECLSPEC_UUID("688e1a58-5094-47c8-adc8-fbcea60ae92b") DECLSPEC_NOVTABLE IDWriteTextAnalysisSource : public IUnknown
{
    ...
    STDMETHOD(GetTextAtPosition)(
        UINT32 textPosition,
        __deref_out_ecount(*textLength) WCHAR const** textString,
    ...
    STDMETHOD(GetLocaleName)(
        UINT32 textPosition,
        __out UINT32* textLength,
        __deref_out_z WCHAR const** localeName
        ) PURE;
    ...
    STDMETHOD(GetNumberSubstitution)(
        UINT32 textPosition,
        __out UINT32* textLength,
        __deref_out IDWriteNumberSubstitution** numberSubstitution
        ) PURE;
};
...
/// an application should stub the method and return a constant/null and S_OK.
/// </summary>
interface DECLSPEC_UUID("688e1a58-5094-47c8-adc8-fbcea60ae92b") DECLSPEC_NOVTABLE IDWrite2TextAnalysisSource : public IUnknown
{
    /// <summary>
    /// Get a block of text starting at the specified text position.
    /// Returning NULL indicates the end of text - the position is after
    ...
    /// <summary>
    /// Gets the preferred glyph orientation on this run.
    /// </summary>
    STDMETHOD(GetOrientation)(
        UINT_PTR runId,
        UINT32 textOffset,
        __out UINT32* textLength,
        __out DWRITE_READING_DIRECTION* readingDirection, /// LTR/RTL/TTB/BTT
        __out DWRITE_GLYPH_ORIENTATION* glyphOrientation /// upright/CW/CCW/USD
        ) PURE;

    // Advances by the given text length, updating the current run id and
    // offset. This application implementation must advance right past any
    // hidden text or backing store buffer gaps. In the simplest case of a
    // single contiguous text buffer (no runs), the runId should be 0 (it
    // must not be random) and it's just a matter of adding the text length
    // to the current offset. In more complex cases, the text offset can
    // vary per run, and even jump across a buffer gap within a run. In any
    // case, the combination of runId and textOffset must be a uniquely
    // identifying pair - there should not be more than one combination that
    // addresses the same text position.
    STDMETHOD(Advance)(
        UINT32 textLength,
        __inout UINT_PTR* runId,
        __inout UINT32* textOffsetInRun
        ) PURE;

    // Recedes by the given text length, updating the current run id and
    // offset. This application implementation must advance right past any
    // hidden text or backing store buffer gaps.
    STDMETHOD(Recede)(
        UINT32 textLength,
        __inout UINT_PTR* runId,
        __inout UINT32* textOffsetInRun
...
/// overwrites the previously set analysis result of the same range. 
/// </summary>
interface DECLSPEC_UUID("5810cd44-0ca0-4701-b3fa-bec5182ae4f7") DECLSPEC_NOVTABLE IDWrite2TextAnalysisSink : public IUnknown
{
    /// <summary>
    /// Report script analysis for the text range.
    /// </summary>
    ...
    STDMETHOD(SetOrientation)(
        ...
        UINT32 textOffsetInRun,
        UINT32 textLength,
        DWRITE_READING_DIRECTION readingDirection, /// LTR/RTL/TTB/BTT
        DWRITE_GLYPH_ORIENTATION resolvedGlyphOrientation, /// upright/CW/CCW/USD
        BOOL isSideways,
        BOOL isReversed
        ) PURE;
...
/// Analyzes various text properties for complex script processing.
/// </summary>
interface DWRITE_DECLARE_INTERFACE("b7e6163e-7f46-43b4-84b3-e4e6249c365e") IDWrite2TextAnalyzer : public IUnknown
{
    /// <summary>
    /// Analyzes a text range for script boundaries, reading text attributes
    /// from the source and reporting the Unicode script ID to the sink 
    ...
    STDMETHOD(AnalyzeScript)(
        IDWriteTextAnalysisSource* analysisSource,
        UINT_PTR startingRunId,
        UINT32 startingTextOffsetInRun,
        UINT32 textLength,
        IDWriteTextAnalysisSink* analysisSink
        ) PURE;
    ...
    /// <summary>
    /// Analyzes a text range for character orientations,
    ...
    /// <summary>
    /// Inserts spacing between characters (tracking).
    /// </summary>
    /// <returns>
    /// Standard HRESULT error code.
    /// </returns>
    STDMETHOD(ApplyCharacterSpacing)(
        ...
        UINT32 textCount,
        UINT32 glyphCount,
        __in_ecount(textCount) FLOAT const* glyphClusters,
        __in_ecount(glyphCount) FLOAT const* glyphAdvances,
        __in_ecount(glyphCount) DWRITE_GLYPH_OFFSET const* glyphOffsets,
        __out_ecount(glyphCount) FLOAT* modifiedGlyphAdvances,
        __out_ecount(glyphCount) DWRITE_GLYPH_OFFSET* modifiedGlyphOffsets
        ) PURE;

    /// <summary>
    /// Asian typography can center characters for better alignment of columns.
    /// This glyph centering onto a grid would be a suitable task for our level.
    /// * Note LS handles this for IE/Word, but it's just a common thing in 
    ///   Japanese layout.
    /// </summary>
    /// <param name="gridCellWidth">Size of grid cells in DIPs.</param>
    /// <param name="alignCharactersToGrid">Centers each character to its
    ///     own grid cell, which would apply to CJK text. Latin characters
    ///     can also be aligned to grid cells, but more often, whole words
    ///     are centered within the grid cells. Your application can
    ///     specify this flag based on settings, or simply read the default
    ///     from DWRITE_SCRIPT_PROPERTIES::alignsCharactersToGrid.</param>
    /// <param name="glyphCount">Total glyphs, both in and out.</param>
    /// <param name="glyphAdvancesIn">Incoming glyph advances.</param>
    /// <param name="glyphOffsetsIn">Incoming glyph offsets.</param>
    /// <param name="glyphAdvancesOut">Outgoing glyph advances.</param>
    /// <param name="glyphOffsetsOut">Outgoing glyph offsets.</param>
    /// <remarks>
    ...
    /// Standard HRESULT error code.
    /// </returns>
    STDMETHOD(AlignGlyphsToGrid)(
        FLOAT gridCellWidth,
        //DWRITE_MEASURING_MODE measuringMode, // this may be irrelevant?
        BOOL alignCharactersToGrid,
        UINT32 glyphCount,
        __in_ecount(glyphCount) FLOAT const* glyphAdvancesIn,
        __in_ecount(glyphCount) DWRITE_GLYPH_OFFSET const* glyphOffsetsIn,
        __out_ecount(glyphCount) FLOAT* glyphAdvancesOut,
        __out_ecount(glyphCount) DWRITE_GLYPH_OFFSET* glyphOffsetsOut
        );

    /// <summary>
    /// Remaps a range of text, given mirroring and a number substitution.
    /// This is essential for correct font fallback, which needs to know the
    /// actual remapped characters to test for, not the original characters.
    /// This is even more important for number substitution than mirroring,
    /// since mirror pairs generally exist in a font together (you are unlikely
    /// to have a mapping for '[' yet not ']').
    /// </summary>
    /// <returns>
    /// Standard HRESULT error code.
    /// </returns>
    /// <remarks>
    /// The input and output can be the same string.
    /// </remarks>
    STDMETHOD(RemapText)(
        __in_ecount(inputTextLength) WCHAR const* textIn,
        UINT32 inputTextLength,
        BOOL isRightToLeft,
        __maybenull IDWriteNumberSubstitution* numberSubstitution,
        __out_ecount(inputTextLength) WCHAR* textOut
        ) PURE;

    /// <summary>
    /// Produces an index mapping from visual index to run index.
    /// This is similar to Uniscribe's ScriptLayout, except that it does not
    /// require you to copy bidi levels out to a temporary byte buffer, and
    /// it only puts a mapping from visual indices to run indices, since this
    /// is the common case.
    /// </summary>
    /// <param name="bidiLevels">Array of bidi levels.</param>
    /// <param name="bidiLevelsStride">Byte stride between bidi levels.
    ///     This avoids the need to copy them out to a temporary array.</param>
    /// <param name="runFirstIndex">Index of first index, which is added to
    ///     each entry in visualToRunIndexOrdering.</param>
    /// <param name="runCount">Total run indices.</param>
    /// <param name="visualToRunIndexOrdering">The result index mapping,
    ///     mapping a visual index (0 to runCount - 1) to the run index.</param>
    /// <returns>
    /// Standard HRESULT error code.
    /// </returns>
    STDMETHOD(GetBidiVisualOrdering)(
        __in_bcount(runCount * bidiLevelsByteStride) UINT8 const* bidiLevels,
        __in_range(>, 0) UINT32 bidiLevelsByteStride,
        UINT32 runFirstIndex,
        UINT32 runCount,
        __out_ecount(runCount) UINT32* visualToRunIndexOrdering
        ) PURE;

    // Given a script and reading direction, it returns the most natural
    // orientation (similar to pango_gravity_get_for_script()).
    // todo: is this really sufficient? script alone is only useful for
    //       strong scripts. For common scripts, you need the character code,
    //       in which case you might as well just use the TextAnalyzer. Though,
    //       it's still useful in the more generic case. Would passing
    //       the character code be more useful? Internally, the mapping stores
    //       based on character code anyway.
    STDMETHOD(GetNaturalGlyphOrientation)(
        DWRITE_SCRIPT_ANALYSIS scriptAnalysis,
        DWRITE_READING_DIRECTION readingDirection,
        __out DWRITE_GLYPH_ORIENTATION* naturalGlyphOrientation
        ) PURE;

    // Given a script, reading direction, and *preferred* glyph orientation,
    // it returns the allowed orientation (similar to pango_gravity_get_for_script()).
    // todo: is this really sufficient? script alone is only useful for
    //       strong scripts. For common scripts, you need the character code,
    //       in which case you might as well just use the TextAnalyzer. Though,
    //       it's still useful in the more generic case?
    STDMETHOD(GetAllowedGlyphOrientation)(
        DWRITE_SCRIPT_ANALYSIS scriptAnalysis,
        DWRITE_READING_DIRECTION readingDirection,
        DWRITE_GLYPH_ORIENTATION preferredGlyphOrientation,
        __out DWRITE_GLYPH_ORIENTATION* resolvedGlyphOrientation
        ) PURE;

    /// <summary>
    /// Returns the respective transform for a given glyph orientation and,
    /// reading direction, decomposing them into the isSideways and isReversed
    /// flags and needed transform for the DrawGlyphRun call.
    ///
    /// There is a constant number of these matrices, and they never change
    /// between calls. So avoid a copy out and just return the matrix directly.
    /// </summary>
    /// <remarks>
    /// The returned transform is has no scaling or translation, so the
    /// diagonals are always equal in magnitude, and dx and dy are always zero.
    /// </remarks>
    /// <returns>
    /// Standard HRESULT error code.
    /// </returns>
    STDMETHOD(GetTransformFromGlyphOrientation)(
        DWRITE_READING_DIRECTION readingDirection,
        DWRITE_GLYPH_ORIENTATION glyphOrientation,
        __out DWRITE_MATRIX const** transform,
        __out BOOL* isSideways,
        __out BOOL* isReversed
        ) PURE;

    /// <summary>
    /// Returns information about a script, such as the public id, useful for
    /// when the application needs to publicly know more about the script for
    /// purposes of caret navigation, maximum cluster size, justification...
    ///
    /// * This need was highlighted by the WPF DCR, which previously took a
    ///   depedency on our internal script enum values.
    ///
    /// This is essentially an updated version of Uniscribe's ScriptGetProperties
    /// function.
    ///
    /// http://msdn.microsoft.com/en-us/library/dd368553(v=VS.85).aspx
    /// </summary>
    /// <param name="scriptAnalysis">Script analysis from itemizer.</param>
    /// <returns>
    /// Pointer to script properties. This function cannot fail. If the given
    /// script is unknown, it returns the properties for the unknown script.
    /// </returns>
    STDMETHOD_(DWRITE_SCRIPT_PROPERTIES const*, GetScriptPropertiesFromScript)(
        DWRITE_SCRIPT_ANALYSIS scriptAnalysis
        ) PURE;

    /// <summary>
    /// Returns the script for a given codepoint.
    ///
    /// * Similar to ICU's uscript_getScript.
    /// </summary>
    /// <param name="character">Unicode character to look up.</param>
    /// <returns>
    /// Script analysis for codepoint. This function cannot fail. If the given
    /// codepoint is unknown (perhaps a codepoint that was later allocated but
    /// was unassigned in earlier Unicode versions), it returns the properties
    /// for the unknown script.
    /// </returns>
    STDMETHOD_(DWRITE_SCRIPT_ANALYSIS, GetScriptFromCharacter)(
        UINT32 character
        ) PURE;

    /// <summary>
    /// Returns the script analysis from a public ISO four letter code.
    ///
    /// http://unicode.org/iso15924/iso15924-codes.html
    /// </summary>
    /// <param name="scriptCode">Unique four letter code.</param>
    /// <param name="scriptAnalysis">Script analysis from itemizer.</param>
    /// <returns>
    /// Standard HRESULT error code.
    /// </returns>
    STDMETHOD(GetScriptFromScriptCode)(
        UINT32 scriptCode,
        __out DWRITE_SCRIPT_ANALYSIS* scriptAnalysis
        ) PURE;

    /// <summary>
    /// Returns the script analysis from a public ISO script number.
    ///
    /// http://unicode.org/iso15924/iso15924-codes.html
    /// </summary>
    /// <param name="scriptNumber">Unique number 0-999.</param>
    /// <param name="scriptAnalysis">Script analysis from itemizer.</param>
    /// <returns>
    /// Standard HRESULT error code.
    /// </returns>
    STDMETHOD(GetScriptFromScriptNumber)(
        UINT32 scriptNumber,
        __out DWRITE_SCRIPT_ANALYSIS* scriptAnalysis
        ) PURE;

    /// <summary>
    /// Converts the glyph properties returned from shaping into basic
    /// justification opportunities, which the application can pass on
    /// directly or tweak first. This is essentially the first half of
    /// the ScriptJustify function just for convenience of simpler or
    /// migrating apps. Capable clients like Internet Explorer and Word
    /// produce the expansion opportunities directly themselves.
    ///
    /// After calling this to get the expansion opportunities, call
    /// JustifyGlyphAdvances to perform the justification distribution,
    /// and InsertJustificationGlyphs before drawing or persisting.
    ///
    /// RFC:
    ///     Do we need to allow any additional customization?
    ///     For example, Latin allows both interword and intercharacter.
    ///     It should be sufficient to support the default case,
    ///     similar to CSS justify, and just allow the client to
    ///     customize cases themselves.
    ///
    ///     Alternately, we could accept some sort of a prioritized
    ///     list of expansion points (such as kashida first, then
    ///     interword, then intercharacter...). Though, this seems more
    ///     complicated than needed. Anyone needing this much
    ///     customization can implement this themselves; and, there
    ///     is nothing stopping them from tweaking the results output.
    /// </summary>
    /// <param name="glyphCount">Total incoming glyph count.</param>
    /// <param name="fontEmSize">Size of font for this run.</param>
    /// <param name="scriptAnalysis">Script analysis for this run. One should
    ///     also be able to pass 0 for default justification for common script.</param>
    /// <param name="justificationOpportunities">Priorities and constraints
    ///     for each glyph.</param>
    /// <param name="glyphPropertiets">Per-glyph properties from GetGlyphs.</param>
    /// <returns>
    /// Standard HRESULT error code.
    /// </returns>
    STDMETHOD(GetJustificationFromGlyphProperties)(
        UINT32 glyphCount,
        FLOAT fontEmSize,
        DWRITE_SCRIPT_ANALYSIS scriptAnalysis,
        __in_ecount(glyphCount) DWRITE_SHAPING_GLYPH_PROPERTIES* glyphProperties,
        __out_ecount(glyphCount) DWRITE_JUSTIFICATION_OPPORTUNITY* justificationOpportunity
        ) PURE;

    /// <summary>
    /// Justifies the glyph advance widths according to the given priorities
    /// and constraints. It distributes extra slack space in the line for each
    /// priority level until it uses all the slack space or exceeds the limits.
    /// Any leftover will be distributed into glyphs that allow residual
    /// expansion. If there are none, the text width will be thinner than the
    /// line width (which would then be aligned according to current alignment).
    ///
    /// This only increases the advance width. It does not insert any of the
    /// needed glyphs for elongation or substitute alternate glyphs. For scripts
    /// that use repeated inserted characters, call InsertJustificationGlyphs.
    /// </summary>
    /// <param name="lineWidth">Width of the line in DIPs.</param>
    /// <param name="glyphCount">Total glyph count.</param>
    /// <param name="justificationOpportunities">Priorities and constraints
    ///     for each glyph</param>
    /// <param name="glyphAdvances">Incoming array of glyph advances.</param>
    /// <param name="glyphOffsets">Incoming array of glyph offsets.</param>
    ...
    /// </remarks>
    /// <remarks>
    /// A minimum glyph-segment width parameter (like ScriptJustify's iMinKashida)
    /// is not needed, since justificationOpportunities contain the minimum width.
    /// </remarks>
    /// <returns>
    /// Standard HRESULT error code.
    /// </returns>
    STDMETHOD(JustifyGlyphAdvances)(
        FLOAT lineWidth,
        UINT32 glyphCount,
        __in_ecount(glyphCount) DWRITE_JUSTIFICATION_OPPORTUNITY* justificationOpportunities,
        __in_ecount(glyphCount) FLOAT const* glyphAdvances,
        __in_ecount(glyphCount) DWRITE_GLYPH_OFFSET const* glyphOffsets,
        __out_ecount(glyphCount) FLOAT* justifiedGlyphAdvances,
        __out_ecount(glyphCount) DWRITE_GLYPH_OFFSET const* justifiedGlyphOffsets
        ) PURE;

    /// <summary>
    /// Pads the glyph run with as many repeated glyph segments as is needed
    /// to fill the difference between the nominal advances and justified
    /// advances. This is done by, for each glyph, taking the difference
    /// and inserting the multiple of glyph segments to fill it.
    ///
    /// This replaces the auto-magic behavior of Uniscribe's ScriptTextOut.
    /// Rather than having the glyphs be transiently produced and discarded,
    /// without any way to get at them, this allows the caller to actually
    /// acquire the glyphs. These can then be rendered (to *any* target, not
    /// just GDI ExtTextOut), optionally cached in layout, or persisted to a
    /// file (as XPS would do).
    ///
    /// This only needs to be called for scripts that use glyph extension for
    /// justification (where the default glyph is not space). Other forms of
    /// justification can just use JustifyGlyphAdvances without further action,
    /// since adjusting the advances and offsets alone is sufficient.
    /// </summary>
    /// <param name="expansionGlyphId">Glyph segment to insert, such as the
    ///     kashida for Arabic or nirugu for Mongolian. It can be any valid
    ///     glyph id, or it can come from GetDefaultJustificationCharacterFromScript.</param>
    /// <param name="expansionGlyphWidth">Width of the glyph segment to insert.</param>
    /// <param name="textCount">Total text count. This is only relevant when
    ///     a clustermap is passed. Otherwise, it is entirely unused.</param>
    /// <param name="glyphCount">Total incoming glyph count.</param>
    /// <param name="maxGlyphCount">Maximum outgoing glyph count. If this value
    ///     is too small, and the output buffers are non-null, you will get an
    ///     insufficient buffer error.</param>
    /// <param name="glyphIndices">Array of glyph indices.</param>
    /// <param name="glyphAdvances">Array of glyph advance widths.</param>
    /// <param name="justifiedGlyphAdvances">Justified advance widths, possibly
    ///     from an earlier call to JustifyGlyphAdvances or manual adjustment.</param>
    /// <param name="justifiedGlyphOffsets">Array of glyph offsets. This either
    ///     came from a call to JustifyGlyphAdvances or manual adjustment.
    ///     This may be null for very simple text.</param>
    /// <param name="clusterMap">Mapping from text array index to glyph array index.</param>
    /// <param name="updatedGlyphCount">New glyph count including insertions.
    ///     This count is always returned as the actual needed count, even if
    ///     greater than the maximum glyph count.</param>
    /// <param name="updatedGlyphIndices">Array of modified indices.</param>
    /// <param name="updatedGlyphAdvances">Array of modified advances.</param>
    /// <param name="updatedGlyphOffsets">Array of modified offsets. This
    ///     may be null for simple languages with no offsets.</param>
    /// <param name="updatedClusterMap">Modified mapping from text offset to
    ///     glyph. Note this does not change the size (since the original text
    ///     is still the same size). It only adjusts the clusters to point to
    ///     the correct glyphs. If this is solely being used for rendering,
    ///     the update clustermap is not needed. If used for persisting the
    ///     glyph run or for selection, this is relevant.</param>
    /// <remarks>
    /// This is meant to be called per-run, not across multiple glyph runs.
    /// Each run must be the same script and same font.
    /// </remarks>
    /// <remarks>
    /// The input and output arrays can be the same.
    /// </remarks>
    ...
    /// Standard HRESULT error code.
    /// </returns>
    STDMETHOD(InsertRepeatedJustificationGlyph)(
        UINT16 expansionGlyphId,
        FLOAT expansionGlyphWidth,
        UINT32 textCount,
        UINT32 glyphCount,
        UINT32 maxGlyphCount,
        __in_ecount(glyphCount) UINT16 const* glyphIndices,
        __in_ecount(glyphCount) FLOAT const* glyphAdvances,
        __in_ecount(glyphCount) FLOAT const* justifiedGlyphAdvances,
        __in_ecount(glyphCount) DWRITE_GLYPH_OFFSET const* justifiedGlyphOffsets,
        __in_ecount_opt(textCount) UINT16 const* clusterMap,
        __out_range(glyphCount, maxGlyphCount) UINT32* updatedGlyphCount,
        __out_ecount_opt(updatedGlyphCount) UINT16* updatedGlyphIndices,
        __out_ecount_opt(updatedGlyphCount) FLOAT* updatedGlyphAdvances,
        __out_ecount_opt(updatedGlyphCount) DWRITE_GLYPH_OFFSET const* updatedGlyphOffsets,
        __out_ecount_opt(textCount) UINT16* updatedClusterMap
        ) PURE;


    // calculate glyph advances given caret position
//todo:

    // calculate black box of range of glyphs
//todo:
...
//todo:
    /*
    http://msdn.microsoft.com/en-us/library/dd368567(VS.85).aspx
...
struct DWRITE_GLYPH_RUN
{
    ...
    /// This size is now normalized to em size, regardless of original font scaling.
    /// </summary>
    FLOAT fontEmSize;
...
/// <summary>
/// The *complete* glyph run, including all the needed information.
/// </summary>
struct DWRITE2_GLYPH_RUN : public DWRITE_GLYPH_RUN
{
    /// <summary>
    ...
    /// <summary>
    /// Measuring method for glyphs in the glyph run.
    /// </summary>
    DWRITE_MEASURING_MODE measuringMode;
};
...
/// <summary>
/// Run of text properties on a range, retrieved the IDWriteParagraphAnalysisCache.
/// </summary>
struct DWRITE2_TEXT_RUN
{
    /// <summary>
    /// The physical font face to draw with.
    /// </summary>
    ...
    /// <summary>
    /// If true, the glyph run is drawn the opposite direction, such as Hebrew,
    /// Arabic, or Ogham as bottom-to-top.
    /// </summary>
    // RFC: Better name?
    //      I was thinking of the LS Reverse object at the time, and I want it
    //      to be consistently named with whatever we produce from the
    //      orientation analysis. Though, RTL folks may not want to think of
    //      their script as being "reversed".
    //
    //      Using "RTL" here is wrong though. After all, Ogham is left-to-right
    //      when horizontal, but bottom-to-top in vertical; and Arabic is
    //      top-to-bottom on book spines, yet still reverse glyph coordinate
    //      order from Latin. Basically, we just need a name that generically
    //      means the glyphs proceed in (the generally opposite) decreasing
    //      font coordinates instead of increasing font coordinates.
    //
    //      Note, something interesting I realized after all this looking into
    //      text directionality is that bidi level is != ltr/rtl. Sadly a lot
    //      of our code conveniently assumes that bidiLevel & 1 automatically
    //      means isRtl, you can have cases where glyphs are shaped in
    //      increasing order (LTR essentially), yet, the bidi level and layout
    //      proceeds the opposite direction! That is why I separated the two
    //      fields here. Too bad the XPS spec bakes this into the bidi level
    //      which is now part of our glyph run.
    UINT32 isReversed           : 1;

    /// <summary>
    /// If true, the text is not visible.
    /// </summary>
    UINT32 isHidden             : 1;

    UINT32 isNumberSubstituted  : 1;
    ...
    /// Additional shaping requirement of text.
    /// </summary>
    DWRITE_SCRIPT_SHAPES shapingRequirement;
};
...
/// <summary>
/// The DWRITE_TEXT_DECORATION structure contains about the size and placement of 
/// text decorations such as underlines and strikethroughs. All coordinates are
/// in device independent pixels (DIPs).
/// </summary>
struct DWRITE_TEXT_DECORATION
{
    /// <summary>
    ...
    /// <summary>
    /// Thickness of the text decoration, measured perpendicular to the
    /// baseline.
    /// </summary>
    FLOAT thickness;
    ...
    /// <summary>
    /// Flow direction of the text associated with the decoration. This 
    /// value is used to interpret whether the thickness value advances top to
    /// bottom, left to right, or right to left.
    /// </summary>
    DWRITE_FLOW_DIRECTION flowDirection;
    ...
    /// <summary>
    /// The measuring mode can be useful to the renderer to determine how
    /// decoration are rendered, e.g. rounding the thickness to a whole pixel
    /// in GDI-compatible modes.
    /// </summary>
    DWRITE_MEASURING_MODE measuringMode;
};
...
struct DWRITE2_LINE_METRICS
{
    ...
    /// <summary>
    /// Distance from the top of the line to its dominant baseline.
    /// </summary>
    FLOAT baseline;
    ...
    /// <summary>
    /// The line is trimmed at least partially.
    /// </summary>
    UINT32 isTrimmedPartially   : 1;

    /// <summary>
    /// The line is trimmed entirely (such as found at the bottom of
    /// vertically trimmed text)
    /// </summary>
    UINT32 isTrimmedEntirely    : 1;
...
struct DWRITE2_CLUSTER_METRICS : public DWRITE_CLUSTER_METRICS
{
    /// <summary>
    /// The height of the the cluster.
    /// </summary>
    FLOAT height;
};
...
interface IDWrite2TextRenderer;
...
/// such as underlines.
/// </summary>
interface DWRITE_DECLARE_INTERFACE("ef8a8135-5cc6-45fe-8825-c5a0724eb820") IDWrite2TextRenderer : public IDWriteTextRenderer
{
    /// <summary>
    /// IDWriteTextLayout::Draw calls this function to instruct the client to draw
    /// a decoration, such as underline, strikethrough, overline, or emphasis mark.
    /// </summary>
    /// <param name="clientDrawingContext">The context passed to 
    /// IDWriteTextLayout::Draw.</param>
    /// <param name="baselineOriginX">X-coordinate of the baseline.</param>
    ...
    /// </returns>
    /// <remarks>
    /// A single decoration can be broken into multiple calls, depending on
    /// how the formatting changes attributes. If font sizes/styles change
    /// within an underline, the thickness and offset will be averaged
    /// weighted according to characters.
    /// </remarks>
    STDMETHOD(DrawTextDecoration)(
        __maybenull void* clientDrawingContext,
        FLOAT baselineOriginX,
        FLOAT baselineOriginY,
        DWRITE2_TEXT_RENDERER_ELEMENT decorationType,
        __in_opt DWRITE_GLYPH_RUN const* glyphRun,
        __in DWRITE_GLYPH_RUN_DESCRIPTION const* glyphRunDescription,
        __in DWRITE_TEXT_DECORATION const* decorationInformation,
        __maybenull IUnknown* clientDrawingEffect
        ) PURE;
};
...
/// <summary>
/// Cache of the paragraph formatting.
/// </summary>
/// <remarks>
/// This interface is single threaded.
/// </remarks>
interface DWRITE_DECLARE_INTERFACE("53737037-6d14-410b-9bfe-0b182bb70962") IDWriteParagraphAnalysisCache : public IUnknown
{
    // Number of runs within the analysis.
    STDMETHOD_(UINT32, GetRunCount)() PURE;

    // Gets the type of this run.
    STDMETHOD(GetRunType)(
        UINT32 runIndex,
        __out DWRITE_RUN_FORMAT_TYPE* type,
        __out UINT_PTR* runId,
        __out UINT32* textLength
        ) PURE;

    // Finds the run index of the given client's run id & offset.
    STDMETHOD(GetRunIndex)(
        UINT_PTR runId,
        UINT32 textOffsetRelativeToRunId,
        __out UINT32* runIndex,
        __out UINT32* textOffsetRelativeToRunIndex
        ) PURE;

    // Finds the client's run id & offset given a run index and offset.
    STDMETHOD(GetRunId)(
        UINT32 runIndex,
        UINT32 textOffsetRelativeToRunIndex,
        __out UINT_PTR* runId,
        __out UINT32* textOffsetRelativeToRunId
        ) PURE;

    // Gets the text analysis of the indexed run.
    STDMETHOD(GetTextRun)(
        UINT32 runIndex,
        __out DWRITE2_TEXT_RUN* textRun
        ) PURE;
};


/// <summary>
/// A single line of text, which is basically just a list of glyph runs
/// at various locations.
///
/// Unlike the larger layout object, the line has no formatting and has no
/// setters. It gets all its formatting information from the backing store
/// of the client. It is also created immediately, with no lazy
/// initialization. If you change anything in the backing store, the line
/// must be recreated.
///
/// All coordinates are in device independent pixels (DIPs).
/// </summary>
...
/// This interface is single threaded.
/// </remarks>
interface DWRITE_DECLARE_INTERFACE("53737037-6d14-410b-9bfe-0b182bb70962") IDWriteTextLine : public IUnknown
{
    // Gets line metrics, which includes sizing information and various text lengths.
    STDMETHOD_(DWRITE2_LINE_METRICS, GetMetrics)() PURE;

    // Whether the line ended from wrapping, hard return, hyphenation, end of text, inline object...
    // * should this just be part of the line metrics?
    STDMETHOD_(DWRITE_LINE_END_TYPE, GetTypeOfLineEnd)() PURE;

    // Number of runs within line.
    STDMETHOD_(UINT32, GetRunCount)() PURE;

    // Gets the type of this line run.
    STDMETHOD(GetRunType)(
        UINT32 runIndex,
        __out DWRITE_RUN_FORMAT_TYPE* type,
        __out UINT_PTR* runId,
    ...
    // Gets the bounding box of the text/inline object for this run.
    STDMETHOD(GetTextBounds)(
        __out DWRITE_RECT* textBounds
        ) PURE;

    // Gets the ink bounding box of the text/inline object for this run.
    STDMETHOD(GetInkBounds)(
        __out DWRITE_RECT* inkBounds
        ) PURE;
    ...
    // Retrieves the glyph run for a given inline. Note the pointers returned
    // are direct for efficiency, but they are only valid for the lifetime of
    // the line object. If you need them longer, copy them out. There are no
    // drawing effects associated with the glyph run, but you can use the
    // runId returned to map to as many effects as you want.
    STDMETHOD(GetGlyphRun)(
        UINT32 runIndex,
        __out DWRITE2_GLYPH_RUN* glyphRun
        ) PURE;

    // Draws the entire line, optionally drawing only specified elements.
    STDMETHOD(Draw)(
        __maybenull void* clientDrawingContext,
        IDWrite2TextRenderer* renderer,
        DWRITE2_TEXT_RENDERER_ELEMENT renderingElements,
        FLOAT originX,
        FLOAT originY
        ) PURE;
    ...
    /// relative to the top-left of the layout box.
    /// </summary>
...
/// </summary>
/// <remarks>
/// This object is thread-safe and requires synchronization if used on multiple threads.
/// </remarks>
interface DWRITE_DECLARE_INTERFACE("9c906818-31d7-4fd3-a151-7c5e225db55c") IDWrite2TextLayout : public IDWriteTextLayout
{
    /// <summary>
    /// Sets the orientation of glyphs.
    /// </summary>
    ...
    STDMETHOD(SetJustificationMode)(
        DWRITE_JUSTIFICATION_MODE justificationMode,
...
/// The GDI interop interface provides interoperability with GDI.
/// </summary>
interface DWRITE_DECLARE_INTERFACE("1edd9491-9853-4299-898f-6432983b6f3a") IDWrite2GdiInterop : public IDWriteGdiInterop
{
    /// <summary>
    /// This returns the pairs from the old kern table.
    ///
    /// http://www.microsoft.com/typography/otspec/kern.htm
    ///
    /// *** There is good reason to want to deprecate use of the pair kerning
    ///     table, and if the font supports kerning via GPOS, that should be
    ///     used instead. However, a good number of fonts still only support
    ///     (and only ever will support, since they won't be updated) the
    ///     kerning table.
    ///
    /// The number of in-the-box Windows fonts containing both an old
    /// kern table and kern GSUB feature clearly increases each release.
    ///
    /// XP SP2: 4 fonts
    ...
    ///     ariblk.ttf calibri.ttf calibrib.ttf calibrii.ttf calibriz.ttf
    ///     cambria.ttc cambriab.ttf cambriai.ttf cambriaz.ttf Candara.ttf
    ///     Candarab.ttf Candarai.ttf Candaraz.ttf constan.ttf constanb.ttf
    ///     constani.ttf constanz.ttf corbel.ttf corbelb.ttf corbeli.ttf
    ///     corbelz.ttf dokchamp.ttf euphemia.ttf framd.ttf gisha.ttf
    ///     gishabd.ttf himalaya.ttf impact.ttf iskpota.ttf leelawad.ttf
    ///     leelawdb.ttf meiryo.ttc meiryob.ttc monbaiti.ttf moolbor.ttf
    ///     pala.ttf palab.ttf palabi.ttf palai.ttf segoepr.ttf segoeprb.ttf
    ///     segoesc.ttf segoescb.ttf segoeui.ttf segoeuib.ttf segoeuii.ttf
    ///     segoeuiz.ttf tahoma.ttf tahomabd.ttf times.ttf timesbd.ttf
    ///     timesbi.ttf timesi.ttf trebuc.ttf trebucbd.ttf trebucbi.ttf
    ///     trebucit.ttf verdana.ttf
    ///
    ...
    ///     cambriab.ttf cambriai.ttf cambriaz.ttf Candara.ttf Candarab.ttf
    ///     Candarai.ttf Candaraz.ttf constan.ttf constanb.ttf constani.ttf
    ///     constanz.ttf corbel.ttf corbelb.ttf corbeli.ttf corbelz.ttf
    ///     dokchamp.ttf euphemia.ttf framd.ttf gisha.ttf gishabd.ttf
    ///     himalaya.ttf impact.ttf iskpota.ttf iskpotab.ttf KhmerUI.ttf
    ///     KhmerUIb.ttf LaoUI.ttf LaoUIb.ttf leelawad.ttf leelawdb.ttf
    ///     meiryo.ttc meiryob.ttc monbaiti.ttf moolbor.ttf msjh.ttf
    ///     msyh.ttf pala.ttf palab.ttf palabi.ttf palai.ttf segoepr.ttf
    ///     segoeprb.ttf segoesc.ttf segoescb.ttf segoeui.ttf segoeuib.ttf
    ///     segoeuii.ttf segoeuil.ttf segoeuiz.ttf seguisb.ttf seguisym.ttf
    ///     Shonar.ttf Shonarb.ttf tahoma.ttf times.ttf timesbd.ttf
    ///     timesbi.ttf timesi.ttf tradbdo.ttf trado.ttf trebuc.ttf
    ///     trebucbd.ttf trebucbi.ttf trebucit.ttf verdana.ttf
    ///
    /// Brand-new font families as well as some of the most commonly used
    /// fonts in the world (Arial and Times) have both GPOS and kern tables.
    ///
    /// It is hard to define exact behavior that will work for everybody.
    /// Does it make sense to make it per-script or we just cover legacy
    /// font with no kerning lookup at all? Should we skip mark glyphs while
    /// looking for kerning pairs? There is no strict answer for these
    /// questions, so it opens door for incompatible implementations and,
    /// therefore, hacky fonts.
    ///
    /// It is dangerous to combine kern and GPOS lookups. It may seem to
    /// help with some fonts, but will mess up less obvious cases.
    /// I remember how easily we broke font lookups by zeroing widths of
    /// mark glyphs outside of GPOS. Applying kerning may have the same effect.
    ///
    /// It is really easy to generate GPOS lookups out of 'kern' table,
    /// especially with the kern2volt tool. We did this backwards for
    /// ClearType fonts without much effect on overall font size.
    /// FontLab also should be able to generate both from one source.
    ///
    /// RFC: Is the kern table cross-stream bit useful/desired by clients?
    /// </summary>
    /// <param name="maxPairCount">Maximum pair count.</param>
    /// <param name="pairs">Array of pairs.</param>
    /// <param name="isSideways">Requesting the sideways kerning pairs.</param>
    /// <param name="getMinimumValues">Return minimum spacing values (instead
    ///     of the usual pair adjustments.</param>
    /// <param name="actualPairCount">Pair count that exists (not the count
    ///     returned). This will always be zero if the font lacks a kerning
    ///     table (some newer fonts only contain GPOS kerning). It will also
    ///     be zero if the requested kind (sideways or minimum) does not exist
    ///     in the table.</param>
    /// <returns>
    /// Standard HRESULT if error, and the number of actual pairs.
    /// It returns E_NOT_SUFFICIENT_BUFFER if the buffer size is
    /// not large enough.
    /// </returns>
    STDMETHOD(GetKerningTablePairs)(
        UINT32 maxPairCount,
        BOOL retrieveAsCharacters,
        BOOL isSideways,          // horizontal bit is 1
        BOOL getMinimumValues,    // minumum bit is 1
        __out_ecount_opt(maxPairCount) DWRITE_KERNING_PAIR* pairs,
        __out UINT32* actualPairCount
    );
};
...
/// The root factory interface for all DWrite objects.
/// </summary>
interface DWRITE_DECLARE_INTERFACE("b859ee5a-d838-4b5b-a2e8-1adc7d93db49") IDWrite2Factory : public IUnknown
{
    /// <summary>
    /// Creates a cache for paragraph analysis information, which includes
    /// text analysis and font mapping. The analyzed properties are invalidated
    /// when the backing store changes.
    /// </summary>
    /// <param name="runFormatSource">Client backing store for text/formatting
    ///     attributes. The analysis does not hold a reference after creation.</param>
    /// <param name="startingRunId">Starting run id.</param>
    /// <param name="startingTextOffset">Starting text offset, relative to run
    ///     id.</param>
    /// <param name="textLimit">Limit to the size of the entire paragraph.
    ///     Passing UINT_MAX makes it scan until an end-of-text is found.</param>
    /// <param name="sectionCount">Number of sections to split line across. If
    ///     there is only one, the section width is simply the maximum allowed
    ///     width of the line.</param>
    /// <param name="sections">Ranges of sections to split line across.</param>
    /// <returns>
    /// Standard HRESULT error code.
    /// </returns>
    STDMETHOD(CreateParagraphAnalysisCache)(
        IDWriteRunFormatSource* runFormatSource,
        UINT_PTR startingRunId,
        UINT32 startingTextOffset,
        UINT32 textLimit,
        __out IDWriteParagraphAnalysisCache** paragraphAnalysisCache
        ) PURE;

    /// <summary>
    /// Creates a single line of text.
    ///
    /// RFC: Do we want to pass multiple sections so that when lines straddle
    ///      across images, it just works, or do we just pass a width and
    ///      require the client to create two separate lines and piece them
    ///      together?
    ///
    ///      It does complicate the code and test a bit, and since jumping over
    ///      floating images is fairly rare, I'd be fine requiring the client
    ///      to create two separate lines that are thought of as a virtual line.
    ///      We'd need to allow the client to adjust line heights so they both
    ///      align correctly and return consistent line metrics.
    /// </summary>
    /// <param name="runFormatSource">Client backing store for text/formatting
    ///     attributes. The line does not hold a reference after creation.</param>
    /// <param name="paragraphAnalysisCache">Cached text analysis from
    ///     paragraph. The line retains a reference.</param>
    /// <param name="runIdStart">Starting run id.</param>
    /// <param name="textStart">Starting text offset, relative to run id.</param>
    /// <param name="textLimit">Limit to the line length. Passing 0 makes it
    ///     continue until it reaches a hard break, end of text, or the
    ///     line length is longer than the maximum.</param>
    /// <param name="sectionCount">RFC: Number of sections to split line across.
    ///     If there is only one, the section width is simply the maximum allowed
    ///     width of the line.</param>
    /// <param name="sectionWidths">RFC: An array of widths. If there are no breaks
    ///     in the line, then there is only one - the width of the line</param>
    /// <param name="sections">Ranges of sections to split line across.</param>
    /// <returns>
    /// Standard HRESULT error code.
    /// </returns>
    STDMETHOD(CreateTextLine)(
        IDWriteRunFormatSource* runFormatSource,
        IDWriteParagraphAnalysisCache* paragraphAnalysisCache,
        UINT_PTR startingRunId,
        UINT32 startingTextOffset,
        UINT32 textLimit,
        UINT32 sectionCount, // RFC:
        __in_ecount(sectionCount) DWRITE_FLOAT_RANGE* sectionWidths, // RFC:
        __out IDWriteTextLine** textLine
        ) PURE;
};
