...
TextEditor::TextEditor(HWND parentHwnd, __notnull IDWriteEditableTextLayout* textLayout)
{
    ...
    textLayout_.Attach(textLayout, true);
    ...
    pageBackgroundEffect_.Attach(new DrawingEffect(0xFF000000 | D2D1::ColorF::White), true);
    textSelectionEffect_.Attach (new DrawingEffect(0xFF000000 | D2D1::ColorF::LightSkyBlue), true);
}
...
void TextEditor::SetRenderTarget(RenderTarget* target)
{
    ...
    renderTarget_.Attach(target, true);
    PostRedraw();
}
...
void TextEditor::OnKeyPress(UINT32 keyCode)
{
    ...
    case VK_BACK:
        ...
        else if (absolutePosition > 0)
        {
            ...
            {
                wchar_t charBackOne = textLayout_->GetCharacterAt(absolutePosition - 1);
                wchar_t charBackTwo = textLayout_->GetCharacterAt(absolutePosition - 2);
...
DWRITE_TEXT_RANGE TextEditor::GetSelectionRange()
{
    ...
    UINT32 textLength = textLayout_->GetTextLength();
...
{
    // Moves the blinking caret to a new position.
...
void TextEditor::CopyToClipboard()
{
    ...
    if (OpenClipboard(hwnd_))
    {
        if (EmptyClipboard())
        {
            ...
            size_t byteSize         = sizeof(wchar_t) * selectionRange.length;
