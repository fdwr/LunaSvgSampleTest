...
/// follows by an array of TableDirectory structures, each of which points to a
/// particular OpenType table and specifies its type.
/// </summary>
...
struct ScriptRecord
{
    OpenTypeULong   tag;                        // 4-byte script tag identifier
...
{
    OpenTypeULong   tag;                        // 4-byte feature identification tag
...
{
    OpenTypeULong   tag;                        // 4-byte language tag identifier
...
struct ClassContextSubtable : ContextSubtable
{
 ...
 // OpenTypeUShort  classSetOffsets[subClassSetCount];  // Array of offsets to SubClassSet tables - from beginning of Substitution/Positioning table - ordered by class - may be NULL

    struct ClassSet
    {
        OpenTypeUShort  classRuleCount;                         // Number of SubClassRule tables
     // OpenTypeUShort  classRuleOffsets[subClassRuleCount];    // Array of offsets to SubClassRule tables - from beginning of SubClassSet - ordered by preference
    };
    ...
    struct ClassRule
    {
     ...
     // ContextLookupRecord lookupRecords[SubstCount];  // Array of Substitution lookups - in design order
    };
};
...
struct EblcEntry
{
    ...
    LineMetrics     hori;                       // line metrics for text rendered horizontally 
    LineMetrics     vert;                       // line metrics for text rendered vertically 

    OpenTypeUShort  startGlyphIndex;            // lowest glyph index for this size 
    OpenTypeUShort  endGlyphIndex;              // highest glyph index for this size 
...
struct BaseTagList
{
    ...
    // OpenTypeULong tags[baseTagCount]         // Array of 4-byte baseline identification tags-must be in alphabetical order 
};
...
{
    OpenTypeULong  tag;                         // 4-byte feature identification tag - must match FeatureTag in FeatureList 
