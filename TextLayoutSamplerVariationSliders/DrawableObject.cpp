...
const float DrawableObject::defaultWidth = 1200;
const float DrawableObject::defaultHeight = 120;
const float DrawableObject::defaultFontSize = 60.0f;
const float DrawableObject::defaultTabWidth = defaultFontSize * 4;
const uint32_t DrawableObject::defaultFontColor = 0xFF000000;
const uint32_t DrawableObject::defaultLayoutColor = 0x00000000; //-- 0xFF7FFFD4;
const uint32_t DrawableObject::defaultBackColor = 0x00000000; //--0xFFFFFFFF;
...
const Attribute DrawableObject::attributeList[DrawableObjectAttributeTotal] =
{
    ...
    {Attribute::TypeArrayUInteger32,Attribute::SemanticCharacterTags,0            , DrawableObjectAttributeAxisTags, u"axis_tags", u"Axis tags", u"", axisValues, countof(axisValues) },
    {Attribute::TypeArrayFloat32,   Attribute::SemanticNone,         0            , DrawableObjectAttributeAxisValues, u"axis_values", u"Axis values", u"", nullptr, 0 },
    {Attribute::TypeUInteger32,     Attribute::SemanticEnumExclusive,0            , DrawableObjectAttributeDWriteFontFamilyModel, u"dwrite_font_family_model", u"DWrite font family model", u"", dwriteFontFamilyModels, countof(dwriteFontFamilyModels) },
};
...
const Attribute::PredefinedValue DrawableObject::dwriteFontFamilyModels[] = {
    ...
    { uint32_t(DWRITE_FONT_FAMILY_MODEL_WEIGHT_STRETCH_STYLE), u"Weight Stretch Style" },
};
...
bool DrawableObject::IsD2DFunction(DrawableObjectFunction functionType) throw()
{
    // Check whether it's a GDI/GDI+ or DWrite based function.
    static_assert(DrawableObjectFunctionTotal == 12, "Update this switch statement.");
    switch (functionType)
    {
    case DrawableObjectFunctionDirect2DDrawTextLayout:
    case DrawableObjectFunctionDirect2DDrawText:
    case DrawableObjectFunctionDirect2DDrawGlyphRun:
    case DrawableObjectFunctionDrawColorBitmapGlyphRun:
    case DrawableObjectFunctionDrawSvgGlyphRun:
        return true;
    }
    return false;
}
...
{
    // Invalidate the cached face. (&, not &&)
    ...
    if (!customFontFilePath.empty())
    {
        ...
        {
            IFR(dwriteFactory6->CreateFontFileReference(
                ToWChar(customFontFilePath.data()),
                nullptr, // lastWriteTime
                OUT &fontFile
            ));
            IFR(dwriteFactory6->CreateFontFaceReference(
                fontFile,
                fontFaceIndex,
            ...
            IFR(fontFaceReference->CreateFontFace(OUT reinterpret_cast<IDWriteFontFace5**>(&this->fontFace)));
        }
#elif 0
        ComPtr<IDWritePrivateFactory1> privateDWriteFactory;
        ComPtr<IDWriteFontFaceReference> fontFaceReference;
        auto* factory = drawingCanvas.GetDWriteFactoryWeakRef();
        factory->QueryInterface(OUT &privateDWriteFactory);

        std::vector<DWRITE_FONT_AXIS_VALUE_FIXED> axisValuesFixed(axisValues.size());
        if (privateDWriteFactory != nullptr)
        {
            for (uint32_t i = 0; i < tagCount; ++i)
            {
                auto& entry = axisValuesFixed[i];
                entry.axisTag = DWRITE_FONT_AXIS_TAG(axisTags[i]);
                entry.fixedCoordinate = axisValues[i] << 16;
            };

            privateDWriteFactory->CreateFontFaceReference(
                ToWChar(customFontFilePath.data()),
                nullptr, // lastWriteTime
                fontFaceIndex,
                axisValuesFixed.data(),
                static_cast<uint32_t>(axisValuesFixed.size()), // variationAxisCount
                DWRITE_FONT_SIMULATIONS_NONE,
                OUT &fontFaceReference
                );
            fontFaceReference->CreateFontFace(OUT reinterpret_cast<IDWriteFontFace3**>(&this->fontFace));
        }
#else
    ...
    )
{
    ...
    //--hack::d2dRenderTarget->BeginDraw();
    ...
    else
    {
        ...
        for (uint32_t i = 0, ci = cachedGlyphRun.glyphCount; i < ci; ++i)
        {
            ...
            gx += gm.advanceWidth * scaleFactor;
        }
        #endif
