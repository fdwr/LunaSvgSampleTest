...
MainWindow::DialogProcResult CALLBACK MainWindow::DialogProc(HWND hwnd, UINT message, WPARAM wParam, LPARAM lParam)
{
    ...
    case WM_TIMER:
        ...
        else if (wParam == IdcUpdateUi + 1)// hack:::
        {
            ...
            {
                auto& drawableObject = drawableObjects_.front();
                array_ref<uint32_t const> axisTags;
                array_ref<float const> axisValues;
                drawableObject.GetValues(DrawableObjectAttributeAxisTags, OUT axisTags);
                drawableObject.GetValues(DrawableObjectAttributeAxisValues, OUT axisValues);
                std::vector<float> newAxisValues(axisValues.begin(), axisValues.end());
                size_t axisCount = axisTags.size();
                newAxisValues.resize(axisCount);

                uint32_t const ticksPerOscillation = 2000;
                uint32_t tickCount = GetTickCount();
                tickCount = tickCount % (ticksPerOscillation * axisCount);
                float axisIndexValue = (tickCount / float(ticksPerOscillation * axisCount)) * axisCount;

                for (size_t axisIndex = 0; axisIndex < axisCount; ++axisIndex)
                {
                    float ratio = 1.0f - std::abs(axisIndexValue - axisIndex);
                    ratio = std::max(ratio, 0.0f);
                    newAxisValues[axisIndex] = float(int32_t(ratio * 1000));
                }

                DrawingCanvasControl& drawingCanvas = *DrawingCanvasControl::GetClass(GetWindowFromId(hwnd_, IdcDrawingCanvas));
                DrawableObjectAndValues::SetFontAxisValues(drawableObject, drawingCanvas, newAxisValues);
            }
            #endif
...
INT_PTR MainWindow::InitializeMainDialog()
{
    ...
    for (size_t i = 0; i < countof(functionNames); ++i)
    {
        ...
        drawableObject.Set(DrawableObjectAttributeText, u"Variable Fonts");
        //drawableObject.Set(DrawableObjectAttributeFontFamily, u"Decovar");
        drawableObject.Set(DrawableObjectAttributeFontFamily, u"Bahnschrift");
        drawableObject.Set(DrawableObjectAttributeFontSize, u"256");
        drawableObject.Set(DrawableObjectAttributeLabel, u" ");
        //drawableObject.Set(DrawableObjectAttributeAxisTags, u"bldA bldB sklA sklB sklD trmA trmB trmC  trmD trmE trmF trmG  trmK trmL wmx2");
        //drawableObject.Set(DrawableObjectAttributeAxisValues, u"151 350 181 950");
        drawableObject.Set(DrawableObjectAttributeAxisTags, u"wght wdth ital slnt opsz");
        drawableObject.Set(DrawableObjectAttributeAxisValues, u"400 100 0 0 20");
        drawableObject.Set(DrawableObjectAttributeTextColor, u"#FFFFFFA5");
...
void MainWindow::DeferUpdateUi(NeededUiUpdate neededUiUpdate)
{
    ...
    SetTimer(hwnd_, IdcUpdateUi, (neededUiUpdate & NeededUiUpdateImmediate) ? 0 : 50, nullptr);
}
...
struct MainWindow::FontFamilyNameProperties
{
    ...
    std::u16string axisTags;
    std::u16string axisValues;
    float fontSize;
};


HRESULT GetAxisValueRanges(
    IDWriteFont* font,
    _Inout_ MainWindow::FontFamilyNameProperties& fontFamilyNameProperties
    )
{
    fontFamilyNameProperties.axisTags.clear();
    fontFamilyNameProperties.axisValues.clear();
    ...
    uint32_t actualFontAxisRangeCount = fontFaceReference1->GetFontAxisValueCount();
    fontAxisValues.resize(actualFontAxisRangeCount);
    fontFaceReference1->GetFontAxisValues(OUT fontAxisValues.data(), actualFontAxisRangeCount);

    for (uint32_t i = 0; i < actualFontAxisRangeCount; ++i)
    {
        union TagAndString
        {
            uint32_t i;
        ...
        TagAndString tag = { fontAxisValues[i].axisTag };
        char16_t stringTag[4] = { tag.c[0], tag.c[1], tag.c[2], tag.c[3] };

        fontFamilyNameProperties.axisTags.append(std::begin(stringTag), std::end(stringTag));
        fontFamilyNameProperties.axisTags.push_back(' ');

        char16_t buffer[12];
        auto stringValue = to_u16string(int32_t(fontAxisValues[i].value), OUT buffer);

        fontFamilyNameProperties.axisValues.append(stringValue.begin(), stringValue.end());
        fontFamilyNameProperties.axisValues.push_back(' ');
    }
...
HRESULT SetFontFamilyNameProperties(LOGFONT const& logFont, _Out_ MainWindow::FontFamilyNameProperties& fontFamilyNameProperties)
{
    ...
    if (font != nullptr)
    {
        ...
        GetAxisValueRanges(font, OUT fontFamilyNameProperties);
    }
...
HRESULT MainWindow::UpdateDrawableObjectsFromFontFamilyNameProperties(FontFamilyNameProperties const& fontFamilyNameProperties)
{
    ...
    for (auto drawableObjectIndex : selectedDrawableObjectIndices)
    {
        ...
        drawableObject.Set(DrawableObjectAttributeAxisTags, fontFamilyNameProperties.axisTags.c_str());
        drawableObject.Set(DrawableObjectAttributeAxisValues, fontFamilyNameProperties.axisValues.c_str());
...
HRESULT MainWindow::GetSelectedDrawableObject(_Out_ uint32_t& selectedDrawableObjectIndex)
{
    ...
    // the first drawable object if none are selected
    // or an error if no objects exist.
...
MainWindow::DialogProcResult CALLBACK MainWindow::OnNotification(HWND hwnd, int controlId, NMHDR* notifyMessageHeader)
{
    ...
    case IdcDrawingCanvas:
        ...
        {
        case NM_CLICK:
        case WM_MOUSEMOVE: // todo:abuse of message
#if 0 // HACK
            ...
            if (!drawableObjects_.empty())
            {
                ...
                float newAxisValue = 0;

                if (DrawableObjectAndValues::GetFontAxisSliderValue(
                        drawableObjects_.front(),
                        drawingCanvas,
                        click.pt.x,
                        click.pt.y,
                        OUT axisIndex,
                ...
                {
                    if (nmh.code == NM_CLICK || GetKeyState(VK_SHIFT) & 0x80) // Set new one.
                    {
                        previouslyClickedAxisIndex_ = axisIndex;
                    }
                    else // Reuse previous one.
                    {
                        axisIndex = previouslyClickedAxisIndex_;
                    }
                    DrawableObjectAndValues::SetFontAxisValue(
                        drawableObjects_,
                        drawingCanvas,
                        axisIndex,
                        newAxisValue
                    ...
                    DeferUpdateUi(NeededUiUpdateDrawableObjectsListView | NeededUiUpdateDrawableObjectsCanvas | NeededUiUpdateAttributesListView | NeededUiUpdateAttributeValuesEdit | NeededUiUpdateImmediate);
                    //DeferUpdateUi(NeededUiUpdateDrawableObjectsCanvas | NeededUiUpdateImmediate);
                }
            }
#endif
            break;
    ...
    case IddMainWindow:
        {
            ...
            WindowPosition windowPositions[] = {
                ...
                /* 13 */ WindowPosition(GetWindowFromId(hwnd, IdcAnimate), PositionOptionsAlignTop),
            ...
            windowPositionEditText.ClampRect({0x7FFF,40});
            WindowPosition::ReflowGrid(windowPositions, uint32_t(countof(windowPositions)), clientRect, spacing, 0, PositionOptionsNone);
