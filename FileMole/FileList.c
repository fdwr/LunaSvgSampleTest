...
int __stdcall LanList_GetTextAt(unsigned int index, unsigned int column, TCHAR *text);
...
static int __stdcall LanListProc(HWND hwnd, UINT message, long wParam, long lParam)
{
    ...
    case WM_CREATE:
	{
		...
		HeaderInfo.pszText=L"Name";	HeaderInfo.cxy=220;	SendMessage(HeaderHwnd, HDM_INSERTITEM, 255, (LPARAM)&HeaderInfo);
		HeaderInfo.pszText=L"Size";	HeaderInfo.cxy=90;	SendMessage(HeaderHwnd, HDM_INSERTITEM, 255, (LPARAM)&HeaderInfo);
		HeaderInfo.pszText=L"Time";	HeaderInfo.cxy=116;	SendMessage(HeaderHwnd, HDM_INSERTITEM, 255, (LPARAM)&HeaderInfo);
		HeaderInfo.pszText=L"ID/IP";	HeaderInfo.cxy=90;	SendMessage(HeaderHwnd, HDM_INSERTITEM, 255, (LPARAM)&HeaderInfo);
		HeaderInfo.pszText=L"Type";	HeaderInfo.cxy=80;	SendMessage(HeaderHwnd, HDM_INSERTITEM, 255, (LPARAM)&HeaderInfo);
	...
	case WM_PAINT:
	{
		...
		#define text_sizeof MAX_PATH
		TCHAR text[text_sizeof];
		...
		if (LanListSize==0) {
			...
			TextOut(hdc, 3-Left,1, text,
				DefWindowProc(hwnd, WM_GETTEXT, text_sizeof,(LPARAM)text)
			);
			SelectClipRgn(hdc, NULL);
			return false;
		}
		...
		while (true) // really does just two passes
		{
			...
			for (item=LanListTop; item<LanListSize; item++, llp++)
			{
				...
				if (flags & LlfRedraw) {
					...
					ExtTextOut(hdc, col+20,rect.top+1,
						ETO_CLIPPED, &hr[HdrIdxName], text,lstrlen(text), NULL);
...
//
// note:
//	!the text buffer is assumed to be at least MAX_PATH characters.
//	the index passed is not the screen row, but rather the actual unique row index.
int __stdcall LanList_GetTextAt(unsigned int index, unsigned int column, TCHAR *text)
{
	LanListEl *lle;
	static TCHAR sep[]=L",";
	...
	case HdrIdxName:
		...
		StringCopy(lle->name, text, MAX_PATH);
		CharLower(text);
		break;
	case HdrIdxSize:
		wsprintf(&text[16], L"%u", lle->size);
		GetNumberFormat(LOCALE_USER_DEFAULT,0, &text[16],&nfm , text, 16);
		break;
	case HdrIdxTime:
		FileTimeToSystemTime((FILETIME*)&lle->timel, &stime);
		wsprintf(text, L"%04u-%02u-%02u %02u:%02u:%02u", stime.wYear, stime.wMonth, stime.wDay, stime.wHour, stime.wMinute, stime.wSecond);
		break;
	case HdrIdxId:
		...
		case LlfServer:
			...
			wsprintf(text, L"%d.%d.%d.%d", ips[0],ips[1],ips[2],ips[3]);
			break;
			}
		case LlfVolume:
			id=lle->serial;
			wsprintf(text, L"%04X-%04X", id>>16,id&0xFFFF); //lle->serial
			break;
		default:
