...
int __stdcall ResizeBarProc(HWND hwnd, UINT message, long wParam, long lParam);
...
int __stdcall ResizeBarProc(HWND hwnd, UINT message, long wParam, long lParam)
{
    ...
    switch (message) {
    case WM_CREATE:
		CursorHorz=LoadCursor(ProgMemBase,(LPSTR)32644);//IDC_SIZENS);
		CursorVert=LoadCursor(ProgMemBase,(LPSTR)32645);
