...
static void SetButtonValue(unsigned int flags);
static unsigned int TotalChoices(short *text);
static unsigned int __fastcall RetardBGR(unsigned int rgb);
...
WNDCLASSW wcAtrList = {
	...
	0, //lpszMenuName
	L"AttributeList" //lpszClassName
};

WNDCLASSW wcAtrOverlay = {
	CS_NOCLOSE, //style
	...
	0, //hIcon
	0, //hCursor
	(HBRUSH)NULL, //hbrBackground
	0, //lpszMenuName
	L"AttributeOverlay" //lpszClassName
};
...
int __stdcall AtrListProc(HWND hwnd, UINT message, long wParam, long lParam)
{
	...
	case WM_CREATE:
	{
		...
		CursorHand=LoadCursor(ProgMemBase,(LPSTR)IDC_HAND);
		...
		hedit = CreateWindowEx(0, "AttributeOverlay","AttributeOverlay", WS_POPUP|WS_CHILD, 0,0, 100,24, hwnd,NULL, ProgMemBase, &(DefaultAl.al));
	...
	case WM_PAINT:
	{
		...
		const unsigned int *pixels=NULL; //picture to display beside item
		unsigned int clrb, clrt; //color of background and text
				...
				if (!(flags & AlfHidden)) {
					if (flags & AlfRedraw) {
						...
						if (pixels && !(flags & AlfNoCheckImage)) {
							...
							// BITMAP format? First and foremost of all stupidities
							// is the upside down nature of bitmaps. Every other gfx
							// format in the WORLD use sensible top down format. Then
							// there is the inconsistent palette format that has caused
							// me grief with every graphics program I've ever written.
							// They couldn't simply be like the rest of the world and
							// use standard RGB format? NO, they had to be different,
							// had to be dumb, and use BGR format. (rant cooling down...)
							ItemBitmap.pal[0]=RetardBGR(clrb);
							ItemBitmap.pal[1]=RetardBGR(clrt);
	...
	*/
		...
		if ((wParam & 0xFFFF) <= 255) { // choice menu sent message
			if (GetSelectedFlags() && (flags & (AlfTypeMask|AlfDisabled|AlfHidden))==AlfMenu) {
				...
				if ((unsigned)wParam < TotalChoices(al->al[selected].text))
					SetButtonValue((flags & ~AlfCheckedMask) | (wParam<<AlfCheckedRs));
			}
			...
			case IDAL_TOGGLE: 
				if (GetSelectedFlags() && (flags & (AlfTypeMask|AlfDisabled|AlfHidden))==AlfToggle)
					SetButtonValue(flags^AlfChecked);
				return 0;
			case IDAL_COMMAND:
	...
	case WM_CHAR:
		...
		&& (flags & (AlfTypeMask|AlfDisabled|AlfHidden)) == AlfEdit) { // if IS edit, and not disabled, hidden, or locked
			if (wParam >= 32) {
				if (!(flags & AlfNumeric) || (wParam >= 48 && wParam <= 57))
					InsertChar(wParam);
			}
		}
		return 0;
...
//		[item]
//		[traveled]
int __stdcall Seek(unsigned int from, int distance, int flagmask)
{
...
static int __stdcall ResizeOverlay()
{
	...
	if (GetSelectedFlags() && GetFocus()==self && (text=al->al[selected].text)) {
		...
		//SetWindowText(hedit, (LPSTR)text);
...
// note:
//		I used asm simply becaused I borrowed from some other code
//		already finished, not to make it faster.
static void __stdcall DeleteChar()
{
	//selected=al->selected;
...
static int AppendString(HGLOBAL *hmut, short *text, int *sizep)
{
	...
	if (dsize<0) dsize=lstrlenW(dest); // size of exisiting string
...
static void ShowContextMenu(unsigned int item, int x,int y)
{
	...
	if (((signed)item >= 0) && !((flags = al->al[item].flags) & (AlfDisabled|AlfHidden))) {
		if ((flags & AlfTypeMask) == AlfToggle) {
			AppendMenu(menu,MF_STRING,IDAL_TOGGLE,"&Toggle");
			SetMenuDefaultItem(menu,0,TRUE);
		}
		else if ((flags & AlfTypeMask) == AlfButton) {
			AppendMenu(menu,MF_STRING,IDAL_COMMAND,"&Do command");
			SetMenuDefaultItem(menu,0,TRUE);
		}
		//else if ((flags & (AlfTypeMask|AlfFile)) == (AlfEdit|AlfFile)) {
		//		AppendMenu(menu,MF_STRING,MidBrowse,"&Browse");
		//		SetMenuDefaultItem(menu,0,TRUE);
		//}
		AppendMenu(menu,MF_STRING,IDAL_COPY,"&Copy");
		if ((flags & AlfTypeMask) == AlfEdit) { // ensure IS edit and is NOT locked
			if (GetPriorityClipboardFormat(&ClipboardFormats[0],2) > 0)
				AppendMenu(menu,MF_STRING,IDAL_PASTE,"&Paste");
			AppendMenu(menu,MF_STRING,IDAL_CLEAR,"C&lear");
		}
		AppendMenu(menu,MF_SEPARATOR,0,0);
	}
	AppendMenu(menu,MF_STRING,IDAL_COPYALL,"Copy &all");
	AppendMenu(menu,MF_STRING,IDAL_HELP,"&Help");
...
static void ShowChoiceMenu() {
	...
	char name[80];
	short *text = al->al[selected].text;
	...
	if (text && (menu=CreatePopupMenu())) {
		...
		for (mid=0; *text; text+=lstrlenW(text)+1, mid++) {
			...
			AppendMenu(menu,MF_STRING,mid,name);
		}
...
// purpose:
//		counts how many choies are in a menu item.
