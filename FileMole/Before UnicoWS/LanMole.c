...
void RegisterClassU(WNDCLASSW *wc);
...
const char  ProgTitle[] ={ "LanMole " ProgVersion " (Alpha/ßeta)"};
...
{ 0,IDC_LANLIST,	"LanItemList","(List is empty. Choose scan to start.)",	0,50,392,506-50,	WS_CHILD|WS_VISIBLE|WS_VSCROLL|WS_HSCROLL|WS_TABSTOP|WS_GROUP|WS_CLIPCHILDREN, WS_EX_CLIENTEDGE};
ChildWindowStruct ToolBar =
{ 0,IDC_TOOLBAR,	"ToolbarWindow32",NULL,				0,0,4096,20,		WS_CHILD|WS_VISIBLE|TBSTYLE_FLAT|CCS_NOPARENTALIGN|CCS_TOP, 0};
ChildWindowStruct txtPath =
{ 0,IDC_PATH,		"EDIT","Type path here",			0,26,392,20,		WS_CHILD|WS_VISIBLE|WS_TABSTOP|ES_AUTOHSCROLL, WS_EX_CLIENTEDGE};
ChildWindowStruct StatBar =
{ 0,IDC_STATUSBAR,	"msctls_statusbar32", "(status bar unfinished)",	0,300,400,20,		WS_CHILD|WS_VISIBLE, 0};
ChildWindowStruct SidePanel =
{ 0,IDC_ATRLIST,	"AttributeList",NULL,				0,0,180,506-44,		WS_CHILD|WS_VISIBLE|WS_TABSTOP|WS_GROUP|WS_CLIPCHILDREN, WS_EX_CLIENTEDGE};
ChildWindowStruct txtLog =
{ 0,IDC_LOG,		"EDIT","LanMole " ProgVersion "  " ProgDate,	0,300,392,50,		WS_CHILD|WS_VISIBLE|WS_VSCROLL|ES_AUTOVSCROLL|ES_MULTILINE|ES_READONLY, WS_EX_CLIENTEDGE|WS_EX_NOPARENTNOTIFY};
ChildWindowStruct SizePanel =
{ 0,IDC_SIZEPANEL,	"ResizeBar","SizeBarPanel",			0,0,4,200,			WS_CHILD|WS_VISIBLE|SBS_VERT, 0, &SizePanelRi};
ChildWindowStruct SizeLog =
{ 0,IDC_SIZELOG,	"ResizeBar","SizeBarLog",			0,300,200,4,		WS_CHILD|WS_VISIBLE|SBS_HORZ, 0, &SizeLogRi};
...
AttribList QueueAl = {
	...
	AlfSeparatorType,			0, NULL, NULL,
	AlfLabel,					TbiViewScan,L"file: ", L"\\\\peekin\\anime\\test.avi",
...
AttribList PropsAl = {
	...
	AlfSeparatorType,			0, NULL, NULL,
	AlfLabel,					0, L"name: ", L"PEEKIN needs to be a longer name so that I can see it spill off",
	AlfLabel,					0, L"path: ", L"\\\\PEEKIN",
	AlfLabel,					0, L"tree: ", L"Microsoft Network\\PEEKIN",
...
unsigned int ScanStatus; //used for status return values in enumeration functions
SOCKET		 ScanSocket;
unsigned int ScanIp;
...
short ScanDriveW[4] = {65,58,92,0}; //colon and null Unicode
char ScanDriveA[4] = {65,58,92,0}; //ANSI version for stupid GetDriveType/GetVolInfo
...
int APIENTRY WinMain (HINSTANCE hinstance, HINSTANCE hPrevInstance, LPSTR lpCmdLine, int cmdShow)
{
    ...
    /*
	if (!RegisterClassU(&wc))         FatalErrorMessage("Failed to register window class");
    if (!RegisterClassU(&wcLanList))  FatalErrorMessage("Failed to register LAN list class");
	//wcAtrList.lpszClassName=(LPSTR)L"AtrListClass";///
    if (!RegisterClassU(&wcAtrList))  FatalErrorMessage("Failed to register attribute list class");
	*/
	RegisterClassU(&wc);
    RegisterClassU(&wcLanList);
    ///RegisterClassU(&wcDlgPanel);
    RegisterClassU(&wcAtrList);
    RegisterClassU(&wcAtrOverlay);
    RegisterClassU(&wcResizeBar);
	...
	//lf.lfHeight=16;
	//lf.lfCharSet=ANSI_CHARSET;//SHIFTJIS_CHARSET;
	lf.lfFaceName[0]=0;
...
//					DefWindowProc(hwnd, WM_SETICON, true, (LPARAM)LoadIcon(ProgMemBase, (LPSTR)IDI_VIEW));
	...
	*/
	...
	{
		char user[256];
		int size = (unicode) ? sizeof(user)>>2 : sizeof(user);
		if (unicode) {
			WNetGetUserW(NULL, (LPWSTR)user, &size);
			AppendLog(L"\r\nUser name: %ls", user);		
		} else {
			WNetGetUserA(NULL, user, &size);
			AppendLog(L"\r\nUser name: %hs", user);		
		}
	}
...
int __stdcall WndProc(HWND hwnd, UINT message, long wParam, long lParam)
{
	...
	case WM_COMMAND:
		...
		case ID_TEST:
			...
			*/
			/*
			ShowWindow( CreateDialog(ProgMemBase, (LPSTR)IDD_SCAN_FILTERS, NULL, GenericDlgProc), SW_SHOW );
			ShowWindow( CreateDialog(ProgMemBase, (LPSTR)IDD_FILTERS, NULL, GenericDlgProc), SW_SHOW );
			ShowWindow( CreateDialog(ProgMemBase, (LPSTR)IDD_ABOUT, NULL, GenericDlgProc), SW_SHOW );
			ShowWindow( CreateDialog(ProgMemBase, (LPSTR)IDD_INFO, NULL, GenericDlgProc), SW_SHOW );
			ShowWindow( CreateDialog(ProgMemBase, (LPSTR)IDD_DELETE_WARN, NULL, GenericDlgProc), SW_SHOW );
			*/
			/*{ //test character
			...
			*/
			...
			HANDLE file=CreateFile("C:\\FILES.TXT", GENERIC_WRITE, FILE_SHARE_READ, NULL,
			...
			while (true) {
				...
				IdTestInsideLoop:
					if (child >= LanListTotal) {
						MessageBox(hwnd, "LanList child out of bounds", "Ack!", MB_OK);
						break;
					}
					WideCharToMultiByte(CP_ACP, 0, LanAtrs[child].name,-1, textA+depth, sizeof(textA), NULL,NULL);
					WriteFile(file, textA,lstrlen(textA), &dummy,NULL);
		...
		case IdHelpReadme:
			...
			break;
		case IdHelpAbout:
			DialogBoxParam(ProgMemBase, (LPSTR)IDD_ABOUT, MainHwnd, (DLGPROC)AboutDlgProc, 0);
			break;
		case IdFileStart:
		...
		case IdListDefaultFile|ALN_CONTEXT:
			InsertMenu((HMENU)lParam,0,MF_BYPOSITION|MF_STRING, IdListDefaultFile,"&Browse file");
		...
		case IdListDefaultFile:	// show open dialog
		{
			...
			OPENFILENAME ofn = {
				sizeof(OPENFILENAME),
				NULL, ProgMemBase,
				"file topology lists (*.ftl;*.txt)\0*.ftl;*.txt\0" "all files\0*\0", NULL,0,1,
				&name[0],sizeof(name),
				NULL,0, 
				NULL,
				"Choose default file list",
		...
		case IdHeirarchyPath|ALN_CONTEXT:
			InsertMenu((HMENU)lParam,0,MF_BYPOSITION|MF_STRING, IdHeirarchyPath,"&Browse folder");
			SetMenuDefaultItem((HMENU)lParam,0,TRUE);
			break;
		case IdHeirarchyPath:
			// show folder dialog
			MessageBox(hwnd, "browse heirarchy root folder", "<Unfinished>", MB_OK);
			break;
		case IdQueuePath|ALN_CONTEXT:
			InsertMenu((HMENU)lParam,0,MF_BYPOSITION|MF_STRING, IdQueuePath,"&Browse folder");
			InsertMenu((HMENU)lParam,1,MF_BYPOSITION|MF_STRING, IdQueuePathDesktop,"To &desktop");
			InsertMenu((HMENU)lParam,2,MF_BYPOSITION|MF_SEPARATOR, 0,NULL);
			SetMenuDefaultItem((HMENU)lParam,0,TRUE);
			break;
 		case IdQueuePath:
			MessageBox(hwnd, "browse queue folder", "<Unfinished>", MB_OK);
			break;
 		case IdQueuePathDesktop:
			MessageBox(hwnd, "set queue folder to desktop", "<Unfinished>", MB_OK);
			break;
		...
		case IDC_LANLIST|LLN_ENTER:
		{
			...
			if (unicode)
				...
				ShellExecute(hwnd,NULL, (LPSTR)path, NULL,NULL,SW_SHOW);
			break;
		}
    ...
    case WM_CREATE:
	{
		...
		//tbab.hInst=NULL;
		//tbab.nID=(int)LoadImage(ProgMemBase, (LPSTR)IDB_TOOLBAR, IMAGE_BITMAP, 0,0, LR_LOADTRANSPARENT|LR_LOADMAP3DCOLORS);
	...
	///	if (SidePanel.hwnd) PostMessage(SidePanel.hwnd, WM_NCACTIVATE,0,0);
		//return 0;
    default:
		if (unicode) return DefDlgProcW(hwnd, message, wParam, lParam);
		else		 return DefDlgProcA(hwnd, message, wParam, lParam);
    }
}
...
BOOL CALLBACK GenericDlgProc(HWND hwnd, UINT message, long wParam, long lParam) 
{
	...
	{
	case WM_INITDIALOG:
		DefWindowProc(hwnd, WM_SETICON, true, (LPARAM)LoadIcon(ProgMemBase, (LPSTR)1));
		return true;
	case WM_COMMAND:
		switch (wParam)
		{
...
BOOL CALLBACK AboutDlgProc(HWND hwnd, UINT message, long wParam, long lParam)
{
	...
	case WM_INITDIALOG:
	{
		...
		char textA[1024]; //ANSI text buffer
		...
		hsrc=FindResource(ProgMemBase, (LPSTR)1, (LPSTR)RT_VERSION);
		...
		*/
		...
		comment = (short*)LoadResource(ProgMemBase, FindResource(ProgMemBase, (LPSTR)1, (LPSTR)RT_VERSION))+88;
...
// computer just left the network, was in sleep mode, or is waiting on
// the CD drive to start. It uses a	critical section for synchronization.
//
...
ScanThread_Sleep:
	while (ScanMode & ScanModePaused) {
		...
		AppendLog(L"\r\nscanning thread waiting (%d items)", LanListTotal);
...
////// actually scanning object
	...
	case LlfDrive:
		...
		if (!ScanStatus) { // false means getvol failed
			AppendLog(L"\r\ndrive error: %ls", &ScanPath.path);
			LanAtrs[ScanParent].flags |= LlfError;
			goto ScanThread_EnumExit;
		}
		if (ScanAddItem(ScanFd.cFileName, ScanFileUnicode))
		{
	...
	case LlfShare: //share on remote server
		//ScanFirstFile(L"\\\\PEEKIN\\VIDEO\\*");
		//ScanFirstFile(L"\\\\ANGELFEESHY\\MUSIC FILES\\*");
		//ScanFirstFile(L"D:\\NET\\CONTENT.IE5\\NXUVWHUZ\\*");
		//ScanFirstFile(L"E:\\Users\\Peekin\\Local Settings\\Temporary Internet Files\\Content.IE5\\CDUB81I3\\*");
		//ScanFirstFile(L"\\\\CRAZYMIR\\MY MUSIC\\*");

		ScanFirstFile(ScanPath.path);
		debugwrite("folder=%ls",ScanPath.path);
		if (ScanHandle==INVALID_HANDLE_VALUE) {
			debugwrite("FindFirstFile error ret=%d err=%d", ScanHandle, ScanStatus);
		...
		for (; ScanStatus && !(ScanMode & (ScanModePaused|ScanModeDie)); ScanNextFile()) {
			// word & byte ptrs to the same string
			union {	short *w;	char *b;} fnp;	//filename ptr word/byte
	  ScanThread_File:
			// Skip entry if temporary or offline file (might only work on NTFS :-/ )
			// also skip hidden or system files if specified
			if (ScanFd.dwFileAttributes & ScanFileAtrFilters) continue;
			...
			fnp.w = &ScanFd.cFileName[0];
			if (ScanFileUnicode) {
				for (; *fnp.w=='.'; fnp.w++);	//Unicode words
				if (*fnp.w==0) continue;		//skip to next filename
			} else {
				for (; *fnp.b=='.'; fnp.b++);	//ASCII bytes
				if (*fnp.b==0) continue;		//skip to next filename
			}
	...
	case LlfServer:
		...
		//if (ScanSocket==0) {
			...
			WsIoControl(ScanSocket, FIONBIO, &ScanSocket); // just pass the sockect as a value since it is alway nonzero
		//}
		...
		//ScanNr.lpRemoteName = L"\\\\PEEKIN";
		//ScanParent=3;
		//LanAtrs[ScanParent].name = L"PEEKIN"; //!hack
		//ScanNr.lpProvider=L"Microsoft Windows Network"; //<- true on WinXP
		//if (ScanStatus = WNetOpenEnum(RESOURCE_GLOBALNET, RESOURCETYPE_DISK, RESOURCEUSAGE_ALL, &ScanNr, &ScanHandle) != NO_ERROR) {
		//if (ScanStatus = WNetOpenEnum(RESOURCE_GLOBALNET, RESOURCETYPE_DISK, 0, &ScanNr, &ScanHandle) != NO_ERROR) {
	...
	ScanThread_Root:
		if (ScanStatus) {
			...
			debugwrite("OpenEnum ret=%d err=%d", ScanStatus, GetLastError());
			//char neterr[512], netprov[128];
			//WNetGetLastError(&ScanStatus, &debugwrite_Buffer,128, &debugwrite_Buffer[0],128);
		...
		debugwrite("NetEnum ct=%d ret=%d err=%d", ScanCount, ScanStatus, GetLastError());
		...
		for (count=0; count<ScanCount && !(ScanMode & (ScanModePaused|ScanModeDie)); count++) {
			...
			if (ScanAddItem(name, ScanNetUnicode))
			{
				type <<= LlfTypeShift;
	...
	case LlfComputer:
	{
		...
		for (count=0; drives; count++,drives>>=1) {
			if (drives & 1) {
				ScanDriveA[0] = (char)(name[0] = 'A'+count);
				type=GetDriveType(ScanDriveA);
				debugwrite("drive=%hs type=%d",ScanDriveA,type);
				if (ScanAddItem(name, ScanNetUnicode))
				{
					LanAtrs[ScanChild].timel = ScanTime.dwLowDateTime;
					LanAtrs[ScanChild].timeh = ScanTime.dwHighDateTime;
...
void ScanNetOpenEnum(NETRESOURCEW *EnumNr) {
	...
	if ((ScanStatus=WNetOpenEnumW(RESOURCE_GLOBALNET, RESOURCETYPE_DISK, RESOURCEUSAGE_ALL, EnumNr, &ScanHandle))
...
UseAnsi:
	...
	ScanStatus=WNetOpenEnumA(RESOURCE_GLOBALNET, RESOURCETYPE_DISK, RESOURCEUSAGE_ALL, (struct _NETRESOURCEA *)EnumNr, &ScanHandle);
}
...
//
// In:
//	ScanFileUnicode - returns Unicode if true, otherwise ANSI
//
// Returns:
//	ScanStatus - success/error code
...
void ScanNetResource() {
	if (ScanNetUnicode)
		ScanStatus=WNetEnumResourceW(ScanHandle, &ScanCount, &ScanBuffer, &ScanSize);
	else
		ScanStatus=WNetEnumResourceA(ScanHandle, &ScanCount, &ScanBuffer, &ScanSize);
}

//	Enumerates files on local machine/server in either ANSI or Unicode.
//	If ScanFileUnicode is set, it returns Unicode strings, otherwise ANSI.
//
// Returns:
//	ScanHandle - handle to enumeration resource
//	ScanStatus - success/error code
//	ScanFd - fills file data with info of first file
//	ScanFileUnicode - if Unicode call was successful (only if OS supports it)
void ScanFirstFile(LPWSTR PathW) {
	char PathA[MAX_PATH];
	if (!ScanFileUnicode) goto UseAnsi;
	ScanHandle=FindFirstFileW(PathW, &ScanFd);
	if (ScanHandle != INVALID_HANDLE_VALUE || (ScanStatus=GetLastError()) != 120) return;
	ScanFileUnicode=false;
UseAnsi:
	WideCharToMultiByte(CP_ACP, 0, PathW,-1, PathA,MAX_PATH, NULL,NULL);
	ScanHandle=FindFirstFile((LPSTR)&PathA, (WIN32_FIND_DATAA*)&ScanFd);
	if (ScanHandle == INVALID_HANDLE_VALUE) ScanStatus=GetLastError();
}
...
void ScanNextFile() {
	if (ScanFileUnicode)
		ScanStatus=FindNextFileW(ScanHandle, &ScanFd);
	else
		ScanStatus=FindNextFileA(ScanHandle, (WIN32_FIND_DATAA*)&ScanFd);
}
...
void ScanVolumeInfo(LPWSTR PathW) {
	if (!ScanFileUnicode) goto UseAnsiVolume;
	// attempt Unicode version first
	ScanDriveW[0] = *PathW; //replace first character with drive letter
	debugwrite("drive=%ls",ScanDriveW);
	ScanStatus = GetVolumeInformationW(ScanDriveW, ScanFd.cFileName,MAX_PATH, &ScanVolumeSerial, NULL,NULL, NULL,0);
	if (!ScanStatus && GetLastError()==120) {
		// Unicode failed because OS does not support, so try ANSI
		ScanFileUnicode=false;
  UseAnsiVolume:
		ScanDriveA[0] = (char)*PathW; //replace first character with drive letter
		debugwrite("drive=%hs", ScanDriveA);
		ScanStatus = GetVolumeInformationA(
			(LPSTR)ScanDriveA,
			(LPSTR)ScanFd.cFileName,MAX_PATH,
			&ScanVolumeSerial, NULL,NULL, NULL,0);
		debugwrite("volinfo lbl=%hs #%08X",ScanFd.cFileName, ScanVolumeSerial);
	}
	debugwrite("GetVolInfo ret=%d err=%d", ScanStatus, GetLastError());
}
...
void DisplayHelpTip (int id, int type, HANDLE handle, HANDLE handle2, int x, int y) {
	...
	if (type==HELPINFO_MENUITEM)
	{
		...
		chwnd=NULL;
		do {
			chwnd = FindWindowEx(NULL, chwnd,"#32768",NULL);
			if (!(int)chwnd) return;
	...
	tti.lpszText = text[0] ? text:"No help for this item";
...
void AppendLog(LPWSTR text, ...)
{
	char textA[256], // format control text
		 msg[256*2]; // resulting msg

	SendMessage(txtLog.hwnd, EM_SETSEL, 32766,32766);
	//SendMessageW(txtLog.hwnd, EM_SCROLLCARET, 0,0);
	if (unicode) {
		wvsprintfW((LPWSTR)msg, (LPWSTR)text,  (char*)((&text)+1));
		SendMessageW(txtLog.hwnd, EM_REPLACESEL, FALSE,(LPARAM)msg);
	} else {
		WideCharToMultiByte(CP_ACP, 0, text,-1, textA, sizeof(textA), NULL,NULL);
		wvsprintfA((LPSTR)msg, (LPSTR)textA, (char*)((&text)+1));
		SendMessageA(txtLog.hwnd, EM_REPLACESEL, FALSE,(LPARAM)msg);
	}
}
...
//extern _WinMainCRTStartup() WinMain();
*/
...
static int __stdcall TempRefreshList(unsigned int item)
{
	...
	if (LanListSize<=0) {
		if (LanAtrs[item].child) {
			SetWindowText(LanList.hwnd, "(folder has children but current filters hide them)");
		} else {
			if (LanAtrs[item].flags & LlfEnumerated)
				SetWindowText(LanList.hwnd, "(folder is empty)");
			else if (LanAtrs[item].flags & LlfDenied)
				SetWindowText(LanList.hwnd, "(folder is passworded)");
			else
				SetWindowText(LanList.hwnd, "(folder has not been scanned yet)");
		}
	}
