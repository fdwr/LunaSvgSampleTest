...
HWND g_hoverHwnd = NULL;
RECT g_hoverRect = {0};
unsigned int g_hoverItem = 0;
...
const static UINT32 g_checkedPixels[7]   = {2,6,142,220,248,112,32};        // right arrow
const static UINT32 g_uncheckedPixels[7] = {198,238,124,56,124,238,198};    // right arrow
...
//
static const UINT32* GetItemTypePixels(UINT32 itemFlags, UINT8 value)
{
    if (itemFlags & AttributeList::Item::FlagNoShowMark)
        return NULL;

    switch (itemFlags & AttributeList::Item::FlagTypeMask)
    {
    case AttributeList::Item::FlagMenuType:
        return g_menuPixels;
        break;
...
bool AttributeList::RegisterWindowClass(HINSTANCE hModule)
{
	...
	wcAtrList.hCursor           = LoadCursor(NULL, IDC_ARROW);
	...
	wcAtrOverlay.hCursor        = LoadCursor(NULL, IDC_ARROW);
...
LRESULT CALLBACK AttributeList::InternalWindowProc(HWND hwnd, UINT message, WPARAM wParam, LPARAM lParam)
{
    ...
    case WM_NCCREATE:
    {
        ...
        SetWindowLongPtr(hwnd, GWLP_USERDATA, PtrToUlong(classInstance));
    ...
    case WM_CREATE:
    {
        ...
        if (g_cursorArrow == NULL)
            g_cursorArrow = LoadCursor(0,IDC_ARROW);
        if (g_cursorIbeam == NULL)
            g_cursorIbeam = LoadCursor(0,IDC_IBEAM);
        if (g_cursorHand  == NULL)
            g_cursorHand = LoadCursor(0,IDC_HAND);

        //hoverlay_ = CreateWindowEx(0, L"AttributeListOverlay",L"AttributeListOverlay", WS_CHILD, 0,0, 100,24, GetParent(hwnd),NULL, HINST_THISCOMPONENT, NULL);
        hoverlay_ = CreateWindowEx(0, L"AttributeListOverlay",L"AttributeListOverlay", WS_POPUP|WS_CHILD, 0,0, 100,24, hwnd,NULL, HINST_THISCOMPONENT, NULL);
        if (hoverlay_ != NULL)
        {
    ...
    case WM_PAINT:
    {
        DWORD   ctColor = GetSysColor(COLOR_CAPTIONTEXT),           // caption text
                atColor = GetSysColor(COLOR_HIGHLIGHTTEXT),         // active text
                itColor = GetSysColor(COLOR_INACTIVECAPTIONTEXT),   // inactive text
                stColor = GetSysColor(COLOR_BTNTEXT),               // selected text
                utColor = GetSysColor(COLOR_WINDOWTEXT),            // unselected text
                dtColor = GetSysColor(COLOR_GRAYTEXT),              // disabled text
                abColor = GetSysColor(COLOR_HIGHLIGHT),             // active background
                sbColor = GetSysColor(COLOR_BTNFACE),               // selected background
                ubColor = GetSysColor(COLOR_WINDOW);                // unselected background
                //sbColor = ((abColor & 0xFEFEFE) + (ubColor & 0xFEFEFE)) >> 1; // blend halfway
            ...
            for (unsigned int itemIndex = topIndex_; itemIndex < totalItems_; ++itemIndex)
            {
                ...
                if (!(flags & Item::FlagHidden))
                {
                    if (flags & Item::FlagRedraw)
                    {
                        ...
                        if (hasItemFocus)                       bgColor = abColor;
                        else if (itemIndex == selectedIndex_)   bgColor = sbColor;
                        else                                    bgColor = ubColor;
                        SetBkColor(hdc, bgColor);

                        if (itemType == Item::FlagTitleType)    textColor = (hasFocus) ? ctColor : itColor;
                        else if (flags & Item::FlagDisabled)    textColor = dtColor;
                        else if (hasItemFocus)                  textColor = atColor;
                        else if (itemIndex == selectedIndex_)   textColor = stColor;
                        else                                    textColor = utColor;
                        ...
                        {
                            hlf = g_guiFontTitle;
                            SelectObject(hdc, GetSysColorBrush(hasFocus ? COLOR_ACTIVECAPTION : COLOR_INACTIVECAPTION));
    ...
    case WM_LBUTTONDOWN:
    {
        ...
        if (UpdateHoverItem(y) < UINT_MAX)
        {
            const Item& item = items_[g_hoverItem];
            if (item.IsEnabled())
            {
                switch (item.GetType())
                {
                case Item::FlagEditType:
                {
                    ...
                    SelectGiven(g_hoverItem, (x < g_hoverRect.left) ? SelectionModeResetCaret : SelectionModeNone );

                    __nullterminated const WCHAR* text;

                    if (selectedIndex_ == g_hoverItem
                    &&  x >= g_hoverRect.left
                    &&  (text = item.text) != NULL)
                    {
                        ...
                        unsigned int len = lstrlenW(text) + 1;
                ...
                case Item::FlagMenuType:
                    SelectGiven(g_hoverItem, SelectionModeNone);
    ...
    case WM_RBUTTONDOWN:
        {
            ...
            ShowContextMenu(g_hoverItem, x, y);
        }
        return 0;

    case WM_CONTEXTMENU:
    {
        if ((HWND)wParam != hwnd || !GetSelectedItemFlags(&flags) || GET_X_LPARAM(lParam) != -1)
            return DefWindowProc(hwnd, message, wParam, lParam);
    ...
    case WM_COMMAND:
        {
            ...
            if (id <= 255) // choice menu sent message
            {
                if (GetSelectedItemFlags(&flags) && Item::IsEnabledType(flags, Item::FlagMenuType))
                {
                    if (id < TotalChoices(items_[selectedIndex_].text))
                        SetButtonValue((UINT8)id);
                }
            }
            else if (id >= IdMax) // special parent msg
            {
    ...
    case WM_GETDLGCODE:
        if (wParam == VK_RETURN && GetSelectedItemFlags(&flags))
    ...
    case WM_KEYDOWN:
        ...
        case VK_DOWN:
        case VK_UP:
            Select(Seek(selectedIndex_, (wParam == VK_DOWN) ? 1 : -1, Item::FlagHidden|Item::FlagDisabled));
            break;
        case VK_NEXT:
        case VK_PRIOR:
        ...
        default:
            {
                ...
                if (!GetSelectedItemFlags(&flags) || (flags & (Item::FlagDisabled|Item::FlagHidden)))
                    return 0;
...
int AttributeList::CopyText(unsigned int itemIndex)
{
    ...
    unsigned int textLen = lstrlenW(text);
        ...
        )
    {
        ...
        if (SetClipboardData(CF_UNICODETEXT, hmWideText) != NULL)
            hmWideText = NULL; // now owned by system

        #ifdef ATTRIBUTE_LIST_COPY_ANSI
        __ecount(textLen+1) PSTR ansiText = reinterpret_cast<PSTR>(GlobalLock(hmAnsiText));
        WideCharToMultiByte(CP_ACP|WC_NO_BEST_FIT_CHARS, 0, text,textLen + 1, ansiText, 256, NULL,NULL);
        GlobalUnlock(hmAnsiText);
        if (SetClipboardData(CF_TEXT, hmAnsiText) != NULL)
...
int AttributeList::CopyAllText()
{
    ...
    if (hmWideText != NULL)
    {
        ...
        if (hmAnsiText != NULL)
        {
            ...
            WideCharToMultiByte(CP_ACP|WC_NO_BEST_FIT_CHARS, 0, wideText,int(textLength), ansiText, int(textLength), NULL,NULL);
            GlobalUnlock(hmAnsiText);
            if (SetClipboardData(CF_TEXT, hmAnsiText) != NULL)
                hmAnsiText = NULL; // now owned by system
        }
...
int AttributeList::PasteText()
{
    ...
    if ((hmWideText != NULL)
    && (clipboardText = reinterpret_cast<PWSTR>(GlobalLock(hmWideText))) != NULL)
    {
...
{
    AttributeList::Overlay* overlay = (AttributeList::Overlay*)GetWindowLong(hwnd, GWLP_USERDATA);
...
//
//		Default flagmask is (Item::FlagHidden | Item::FlagSeparator | Item::FlagDisabled)
...
bool AttributeList::ResizeOverlay()
{
    ...
    UINT32 flags;
    if (!GetSelectedItemFlags(&flags) || items_[selectedIndex_].text == NULL || (GetFocus() != hwnd_))
    {
        //DestroyCaret();
    ...
    if (hoverlay_ != NULL)
    {
        ...
        AttributeList::Overlay& overlay = *AttributeList::Overlay::GetClass(hoverlay_);
        SetWindowPos(hoverlay_,NULL, pt.x,pt.y, sz.cx,sz.cy, SWP_NOACTIVATE|SWP_NOCOPYBITS|SWP_NOZORDER);
        SetWindowPos(hoverlay_, NULL, 0,0,0,0, SWP_NOMOVE|SWP_NOSIZE|SWP_NOZORDER|SWP_NOACTIVATE|SWP_SHOWWINDOW);
        ...
        {
            CreateCaret(hoverlay_, NULL, 2, metrics.itemHeight);
...
void AttributeList::SetCaretIndex(unsigned int newIndex)
{
    ...
    if (newIndex > 0)
    {
        ...
        GetTextExtentExPointW(hdc, text,len+1, 0,NULL, &caretX[1], &sz);
...
{
    InvalidateRect(hoverlay_, NULL, FALSE);
}
...
//
bool AttributeList::GetSelectedItemFlags(UINT32* flags)
{
    if (selectedIndex_ >= totalItems_)
    {
        *flags = Item::FlagDisabled | Item::FlagHidden;		
...
unsigned int AttributeList::UpdateHoverItem(int y)
{
    ...
    {
        g_hoverHwnd = hwnd_;
        GetItemTextRect(y, &g_hoverItem, &g_hoverRect);
    }
    return g_hoverItem;
}
...
bool AttributeList::GetItemTextRect(unsigned int itemIndex, int topY, const ListMetrics& metrics, __out RECT* rectOut)
{
    ...
    if (itemIndex < totalItems_) // over valid item
    {
        ...
        else if (GetItemTypePixels(item.flags, item.value) != NULL)
        {
...
void AttributeList::FlagTitlesNeedRedraw()
{
    ...
    {
        Item& item = items_[itemIndex];
        if ((item.flags & Item::FlagTypeMask) == Item::FlagTitleType)
            item.SetRedraw();
    }
}
...
{
    DEBUG_ASSERT(hwnd_ != NULL);
    DEBUG_ASSERT(g_guiFont != NULL);
...
void AttributeList::ScrollBy(int rowDif, UINT32 selectionMode)
{
    ...
    if (selectionMode & SelectionModePage)
    {
        ...
        //Seek(item, (TopDif >= 0) ? -1:1, Item::FlagHidden|Item::FlagDisabled|AlfSeparator);
...
{
    g_hoverRect.bottom = 0; // force mouse recalc upon next mouse move
    ScrollWindow(hwnd_, 0, rowDif * metrics.itemHeight, NULL,NULL);
    SetScrollBars(metrics);
    ResizeOverlay();
}
...
PWSTR AttributeList::GetItemText(unsigned int itemIndex)
{
    ...
    switch (item.flags & Item::FlagTypeMask)
    {
    case Item::FlagButtonType:
    case Item::FlagMenuType:
    case Item::FlagToggleType:
...
void AttributeList::AppendString(
    __deref_ecount(*previousMemoryLen) WCHAR** memoryBlock,
    __nullterminated const WCHAR* const text,
    ...
    )
{
    ...
    size_t textLen     = lstrlenW(text); // new text size
    ...
    if (neededSize > memorySize)
    {
        ...
        HGLOBAL newTarget = GlobalReAlloc(hmTarget, neededSize + 512, GMEM_MOVEABLE);
        if (newTarget == NULL)
            return;
    ...
    __bcount(neededSize) PWSTR memoryText = reinterpret_cast<PWSTR>(*memoryBlock);
...
void AttributeList::ShowContextMenu(unsigned int itemIndex, int x,int y)
{
    ...
    if (!(flags & Item::FlagHidden))
    {
        ...
        UINT menuFlags = MF_STRING | ((flags & Item::FlagDisabled) ? MF_DISABLED : 0);
...
//		nothing, WM_COMMAND is sent to window procedure
//
void AttributeList::ShowChoiceMenu()
{
    DEBUG_ASSERT(selectedIndex_ < totalItems_);

    const Item& item = items_[selectedIndex_];

    HMENU menu;
    __nullterminated const WCHAR* text = item.text;
            
    if (text != NULL
    && ((menu = CreatePopupMenu()) != NULL))
    {
        // build menu choices
        for (int menuId = 0; *text; text += lstrlenW(text)+1, menuId++)
        {
            AppendMenu(menu, MF_STRING, menuId, text);
        }
        SetMenuDefaultItem(menu, item.value, TRUE);

        RECT rect;
        GetItemTextRect(selectedIndex_, &rect);
        rect.left = 0;
...
//		nothing
//
void AttributeList::SetButtonValue(UINT8 value)
{
    DEBUG_ASSERT(selectedIndex_ < totalItems_);
