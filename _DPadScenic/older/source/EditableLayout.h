...
//              but the caller can safely hold the same reference, since the
//              adapter forwards all the calls onward.
//
//----------------------------------------------------------------------------
...
////////////////////////////////////////
// Editable interface that supports text insertion/removal.

interface DWRITE_DECLARE_INTERFACE("3C5138C7-7428-4155-94EC-F310F3ECDE9E") IDWriteEditableTextLayout : public IDWriteTextLayout
{
public:
    struct CaretFormat
    {
    ...
    /// <param name="lengthToInsert">The length of the string.</param>
    ...
    STDMETHOD(InsertTextAt)(
        ...
        __maybenull CaretFormat* caretAttributes = NULL
        ) PURE;
    ...
    /// <param name="lengthToRemove">The number of characters to delete.</param>
    /// <returns>
    /// Standard HRESULT error code.
    /// </returns>
...
////////////////////////////////////////
// Implements the EditableLayout via containment.

class EditableLayout
    :   public ComBase<
            QiList<IDWriteEditableTextLayout,
            QiList<IDWriteTextLayout,
            QiList<IDWriteTextFormat
        > > > >
{
    ComPtr<IDWriteTextLayout> innerLayout_;
    ComPtr<IDWriteTextFormat> textFormat_;
    ComPtr<IDWriteFactory> factory_;
    std::wstring text_;

public:
    EditableLayout(IDWriteTextLayout* textEditLayout, IDWriteFactory* factory, std::wstring text)
    {
        assert(textEditLayout != NULL);
        innerLayout_ = textEditLayout;
        factory_ = factory;
        text_ = text;
        HrException::IfFailed(innerLayout_->QueryInterface(
...
public:
    ...
    STDMETHOD(SetTextAlignment)(DWRITE_TEXT_ALIGNMENT textAlignment)
    {
        return innerLayout_->SetTextAlignment(textAlignment);
    }

    STDMETHOD(SetParagraphAlignment)(DWRITE_PARAGRAPH_ALIGNMENT paragraphAlignment)
    {
        return innerLayout_->SetParagraphAlignment(paragraphAlignment);
    }

    STDMETHOD(SetWordWrapping)(DWRITE_WORD_WRAPPING wordWrapping)
    {
        return innerLayout_->SetWordWrapping(wordWrapping);
    }

    STDMETHOD(SetReadingDirection)(DWRITE_READING_DIRECTION readingDirection)
    {
        return innerLayout_->SetReadingDirection(readingDirection);
    }

    STDMETHOD(SetFlowDirection)(DWRITE_FLOW_DIRECTION flowDirection)
    {
        return innerLayout_->SetFlowDirection(flowDirection);
    }

    STDMETHOD(SetIncrementalTabStop)(FLOAT incrementalTabStop)
    {
        return innerLayout_->SetIncrementalTabStop(incrementalTabStop);
    }
    ...
    {
        return innerLayout_->SetTrimming(trimmingOptions, trimmingSign);
    }
    ...
    STDMETHOD(GetFontCollection)(__out IDWriteFontCollection** fontCollection)
    {
...
public:
    // Adapters for the IDWriteTextLayout functions
    STDMETHOD(SetMaxWidth)(FLOAT maxWidth)
    {
        return innerLayout_->SetMaxWidth(maxWidth);
    }

    STDMETHOD(SetMaxHeight)(FLOAT maxHeight)
    {
    ...
    {
        return innerLayout_->GetMetrics(textMetrics);
    }

    STDMETHOD(GetOverhangMetrics)(__out DWRITE_OVERHANG_METRICS* overhangs)
    {
        return innerLayout_->GetOverhangMetrics(overhangs);
    }
    ...
    STDMETHOD(DetermineMinWidth)(__out FLOAT* minWidth)
    {
        return innerLayout_->DetermineMinWidth(minWidth);
    }
 ...
 public:
    ...
    STDMETHOD(InsertTextAt)(
        ...
        __in_ecount(lengthToInsert) WCHAR const* text,
        UINT32 lengthToInsert,
        __maybenull CaretFormat* caretFormat = NULL
        );
    ...
    STDMETHOD_(wchar_t, GetCharacterAt)(UINT32 position);
...
private:
    ...
    void RecreateInnerLayout(IDWriteTextLayout* oldLayout);
};
