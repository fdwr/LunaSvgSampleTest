...
public:
    ...
    /// Create a CheckedPtr from a pointer and count.
    /// </summary>
    CheckedPtr(T* begin, T* end) throw()
    :   p_(begin),
        count_(static_cast<S>(end - begin))
    {
        assert(size_t(end - begin) <= std::numeric_limits<S>::max());
    }
    ...
    static Self CreateFromBuffer(__in_bcount(count) U* p, SizeType count) throw()
    {
        ...
        static_assert(ALIGNOF(T) == 1, "This function works with uint8 types.");
    ...
    template<typename U>
    bool TryReadAt(size_t offset, __deref_out_bcount(sizeOfU) U** u, size_t sizeOfU = sizeof(U)) throw()
    {
        // Bounds check.
        if (count_ < offset || count_ - offset < sizeOfU)
        {
    ...
    /// </summary>
    template<class U>
    /*__xcount(sizeOfU)*/ U& ReadAt(size_t offset, size_t sizeOfU = sizeof(U))
    {
        U* u;
        if (!TryReadAt(offset, &u, sizeOfU))
        {
    ...
    /// <summary>
    /// Writes a single value of the specified type at the specified byte offset.
    /// Return false if the write is out of range.
    /// </summary>
    template<typename U>
    bool TryWriteAt(size_t offset, __in_bcount(sizeOfU) const U& u, size_t sizeOfU = sizeof(U)) const throw()
    {
        // Bounds check.
        if (count_ < offset || count_ - offset < sizeOfU)
        {
    ...
    /// </summary>
    template<class U>
    /*__xcount(sizeOfU)*/ void WriteAt(size_t offset, const U& u, size_t sizeOfU = sizeof(U)) const
    {
        if (!TryWriteAt(offset, u, sizeOfU))
        {
            FailOutOfRange();
        }
    }
