...
void GetFormattedString(OUT std::wstring& returnString, const wchar_t* formatString, ...) 
{
    ...
    GetFormattedString(returnString, false, formatString, vargs);
...
void AppendFormattedString(IN OUT std::wstring& returnString, const wchar_t* formatString, ...) 
{
    va_list vargs = nullptr;
    va_start(vargs, formatString); // initialize variable arguments
    GetFormattedString(returnString, true, formatString, vargs);
    va_end(vargs); // Reset variable arguments
}
...
void ConvertText(
    ...
    OUT std::wstring& char16text
    )
{
    // This function can only throw if out-of-memory when resizing char16text.
    // If char16text is already reserve()'d, no exception will happen.
    ...
    char16text.resize(utf8text.size());
    ...
    int32_t charsConverted =
        MultiByteToWideChar(
            ...
            OUT const_cast<wchar_t*>(char16text.data()), // workaround issue http://www.open-std.org/jtc1/sc22/wg21/docs/lwg-active.html#2391
            int32_t(char16text.size())
            );

    // Shrink to actual size.
    char16text.resize(charsConverted);
}


void ConvertText(
    array_ref<wchar_t const> char16text,
    OUT std::vector<char>& utf8text
    )
{
    utf8text.resize(char16text.size());
