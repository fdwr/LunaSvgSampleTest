...
.dwReserved0        resd 1
.dwReserved1        resd 1
.cFileName          resb MAX_PATH ;*2 for Unicode
.cAlternate         resb 14 
endstruc
...
; Restores multiple registers, first left to right.
; (prm1, prm2, prm3...)
%macro pop 2-*
  %rep %0
    pop %1
...
; parameter after the return address (esp+4). The names should all be
; preceded by periods if they are to be routine local.
; (dwords prm1, prm2, prm3...)
%macro params 1-*
    %assign Ptr 4
    %rep %0
      %1 equ Ptr                ;assign name to stack offset
      %assign Ptr Ptr+4         ;next parameter
      %rotate 1
    %endrep
%endmacro

; Assigns routine parameter names to stack offsets, relative to a given base.
; The names should all be preceded by periods if they are to be routine local.
; (dwords base, prm1, prm2, prm3...)
%macro paramsrel 2-*
    %assign Ptr %1
    %rep %0-1
      %rotate 1
...
;  height
;)
%macro DefWndControl 9
align 4,db 0
%1:
    ; define a WndControlStruct
    dd 0                        ;reserve dword for window handle
    dd %2                       ;id
    .Id equ %2
    dd %3                       ;class name
%ifstr %4
...
%else
  %ifdef UseConsoleDebug
    %ifndef UseTextConsole
      %error "UseTextConsole must be defined to use UseConsoleDebug"
    %endif
  %endif
...
__SECT__
    %else
       %error "Debug message must be a string"
    %endif
    push dword %%Msg
    call WriteDebugMsg
  %endmacro
...
[section code]
; Writes formula string to any active debugger. Saves all regs and flags. Can
; be called from different threads without crashing, but output might get
; mixed up.
...
%if 0
    ...
    api wvsprintf, .TextBuffer, [esp+36+4], esi
    api OutputDebugString, .TextBuffer
    popfw
    pop ax
    popa
...
WriteDebugMsg:
    ...
    pushf
  %ifdef UseConsoleDebug
    mov eax,[WriteDebugFormula.StdOut]
    test eax,eax
    jnz .HandleValid
    api GetStdHandle, STD_ERROR_HANDLE
    mov [WriteDebugFormula.StdOut],eax
  .HandleValid:
    mov esi,[esp+32+4+4]
    xor ecx,ecx
  .NextChar:
    ...
    jmp short .NextChar
  .Eos:
    api WriteConsole, eax, esi,ecx, WriteDebugFormula.Dummy, NULL
  %else
...
[section bss]
alignb 4
.TextBuffer     resb 1024 ;equ WriteDebugFormula.TextBuffer
...
[section code]
; (eax=message number) (edx=msg ptr; eax)
GetWinMsgName:
    mov edx,WinMsgNames
    xor ecx,ecx
    jmp short .Start
