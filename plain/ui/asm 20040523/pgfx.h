...
////////////////////////////////////////
#ifdef ASM
	PgfxFlags.slowPc	 equ 1<<0	; slow machine so sacrifice quality to speed up
	PgfxFlags.hasMMX	 equ 1<<1	; processor supports feature
	PgfxFlags.useMMX	 equ 1<<2	; user wants to use MMX (only CPU supports)
	PgfxFlags.mulInit	 equ 1<<3	; multiplication look up table initialized
	PgfxFlags.redraw	 equ 1<<8	; something needs redrawing
	PgfxFlags.cursorMove equ 1<<16	; cursor was moved
	PgfxFlags.cursorSet	 equ 1<<17	; new cursor set
	PgfxFlags.cursorHidden equ 1<<18
	...
	PgtLargeCoordinate equ -1>>2
#else //C
enum {
	PgfxFlags_slowPc=1<<0,		// slow machine so sacrifice quality to speed up
	PgfxFlags_hasMMX=1<<1,		// processor supports feature
	PgfxFlags_useMMX=1<<2,		// user wants to use MMX (only CPU supports)
	PgfxFlags_mulInit=1<<3,		// multiplication look up table initialized
	PgfxFlags_redraw=1<<8,		// something needs redrawing
	PgfxFlags_cursorMove=1<<16,	// cursor was moved
	PgfxFlags_cursorSet=1<<17,	// new cursor set
	PgfxFlags_cursorHidden=1<<18
};
...
#ifdef ASM
struc PgtCursor
.col:			resd 1			; current hotspot pixel position
.row:			resd 1
.colOffset:		resd 1			; amount cursor image is offset from actual
.rowOffset:		resd 1
.colPrev:		resd 1			; previous position of cursor + hotspot
.rowPrev:		resd 1
...
typedef struct {
	uint32 col, row;
	uint32 colOffset, rowOffset;
	uint32 colPrev, rowPrev;
	void* PgtLayer;
	void* background;
} PgtCursor;
#endif
...
#ifdef ASM
	extern PgfxDisplay
	extern PgfxDisplay.pixels
	extern PgfxDisplay.wrap
	extern PgfxDisplay.left
	extern PgfxDisplay.top
	extern PgfxDisplay.height
	extern PgfxDisplay.width
	extern PgfxDisplay.clipLeft
	extern PgfxDisplay.clipTop
	extern PgfxDisplay.clipRight
	extern PgfxDisplay.clipBottom
	extern PgfxDisplay.redrawLeft
	extern PgfxDisplay.redrawTop
	extern PgfxDisplay.redrawRight
	extern PgfxDisplay.redrawBottom
	extern PgfxDisplayImage
	extern PgfxCursor
#else	// C
	extern "C" PgtDisplayFull PgfxDisplay;
