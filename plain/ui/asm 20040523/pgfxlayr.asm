...
param sectRight		; pixel offset from right
param sectBottom	; pixel offset from bottom
...
local destDisplay, PgtDisplayPartial_size
;local tempImage ,PgtImage_size
	...
	mov esi,PgfxDisplay
	lea edi,[.destDisplay]
	mov ecx,PgtDisplayPartial_size/4
	rep movsd
...
.Next:	; loop top
	...
	; endif
	; if streching or tiling, but not opaque (like add, sub, mask...)
	;   create temp image
	;   then do operation
	; endif
	; if usealpha flag
...
.Start: ; loop entry
	test dword [esi+PgtLayer.flags],PgtLayer.FlagLast
	jz .Next
...
.BopOpaque:
	...
	call .SetSourceAtrs
	call .ClipImage1
	test dword [esi+PgtLayer.flags],PgtLayer.FlagStrechHorz|PgtLayer.FlagTileHorz|PgtLayer.FlagStrechVert|PgtLayer.FlagTileVert
	jnz .BopOpaqueStrech
.BopOpaqueNormal:
...
.CalcLayerEdges:
	...
	%if (PgtLayer.FlagLeftOffset != 0) || (PgtLayer.FlagLeftOffsetRight > PgtLayer.FlagLeftSize) || (PgtLayer.FlagLeftSize > PgtLayer.FlagLeftPercent)
...
.ClelCheckOffsetRight:
	test eax,~PgtLayer.FlagLeftOffsetRight & PgtLayer.FlagLeftMask
	jnz .ClelCheckSize
	add ebx,edx
	jmp short .ClelCalced
.ClelCheckSize:
...
.DerCheckOffset:
	...
	jmp short .DerCalced
.DerCheckOffsetLeft:
	test eax,~PgtLayer.FlagRightOffsetLeft & PgtLayer.FlagRightMask
	jnz .DerCheckSize
	add ebx,ecx
	jmp short .DerCalced
.DerCheckSize:
...
.CletCheckOffsetBottom:
	test eax,~PgtLayer.FlagTopOffsetBottom & PgtLayer.FlagTopMask
	jnz .CletCheckSize
	add ebx,edx
	jmp short .CletCalced
.CletCheckSize:
...
.ClebCheckOffsetTop:
	test eax,~PgtLayer.FlagBottomOffsetTop & PgtLayer.FlagBottomMask
	jnz .ClebCheckSize
	add ebx,ecx
	jmp short .ClebCalced
.ClebCheckSize:
...
;///////////////////////////////////////
; Clips a normal/tiled/streched image to the destination layer.
...
; (esi=layer ptr)
	...
	test eax,PgtLayer.FlagStrechHorz|PgtLayer.FlagTileHorz
	jnz .Ciht
...
; (eax=flags, esi=layer ptr)
.Ciht:
	; for horizontally tiled or streched images
...
; (esi=layer ptr)
	; clip vertically
	test eax,PgtLayer.FlagStrechVert|PgtLayer.FlagTileVert
	jnz .Civt
...
; (eax=flags, esi=layer ptr)
.Civt:
	; for vertically tiled or streched images
	; clip layer to display extents
	mov edx,[.layerBottom]
	mov ecx,[.layerTop]
	mov edi,edx
