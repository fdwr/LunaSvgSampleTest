...
LRESULT __stdcall AtrListProc(HWND hwnd, UINT message, long wParam, long lParam);
LRESULT __stdcall AtrOverlayProc(HWND hwnd, UINT message, long wParam, long lParam);
...
static LPTSTR __stdcall GetChoiceText(TCHAR* text);
...
static unsigned int TotalChoices(TCHAR* text);
...
LRESULT __stdcall AtrListProc(HWND hwnd, UINT message, long wParam, long lParam)
{
	...
	case WM_CREATE:
	{
		...
		RegisterClass(&wcAtrOverlay); // do again in case not done yet
		hedit = CreateWindowEx(0, AtrOverlayClass,
				AtrOverlayClass, WS_POPUP|WS_CHILD, 0,0, 100,24, hwnd,NULL, ProgMemBase, &(DefaultAl.al));
	...
	case LB_INITSTORAGE: // fills attribute list with items
		...
		// count actual items contained if nonpositive total
		if (al->total == -1) {
			int item;
			for (item = 0; al->al[item].flags != 0; item++);
			al->total = item;
		}
			 ...
			 )
			{
				...
				if (!(flags & AlfHidden)) {
					if (flags & AlfRedraw) {
						...
						if (text = ali->label) {
							len=lstrlen(text);
							TextOut(hdc, x,rect.top+CaretY, text,len);
			...
			if (!(flags & (AlfDisabled|AlfHidden))) {
				...
				case AlfEdit:
					...
					if (x >= HoverRect.left) {
						...
						LPTSTR text = al->al[selected].text;
						...
						if (unicode) {
							...
							len=lstrlen(text)+1;
							if (len>256) len=256;
							GetTextExtentExPoint(hdc, text,len, 0,NULL, &caret[1],&sz);
							caret[0]=0;
	...
	*/
	...
	CopyText:
		...
		if (OpenClipboard(hwnd)) {
			...
			if (hmt = GlobalAlloc(GMEM_MOVEABLE|GMEM_DDESHARE, 256*sizeof(TCHAR)) ) {
				src = GetChoiceText(al->al[selected].text);
				if (!src) dest = al->al[selected].label;
			...
			if (hmt = GlobalLock(GlobalAlloc(GMEM_MOVEABLE|GMEM_DDESHARE,4096)) ) { // if mem alloced
				...
				for (; (signed)item>0; item--,ali++) {
					...
					&& (text = ali->text)) {				// has text
						label = ali->label;
						AppendString(&hmt, label);
						AppendString(&hmt, GetChoiceText(text));
						AppendString(&hmt, T("\r\n"));
					}
				}
	...
	PasteText:
		...
		&& (flags & AlfSizeMask) // ensure edit allows at least one character in buffer
		&& OpenClipboard(hwnd)) {
			...
			if ((dest = al->al[selected].text)
			&& (hmt = GetClipboardData(format))
			&& (src = GlobalLock(hmt)) ) {				
				//if (format==CF_UNICODETEXT) {
				debugwrite("paste text=%s", src);
				len = lstrlen(src);
				if (len > (flags>>AlfSizeRs)) len = flags>>AlfSizeRs;
	...
	case WM_GETDLGCODE:
		if (GetSelectedFlags()) {
			...
			|| GetWindowLong(self, GWL_STYLE) & LBS_NOTIFY)
			{
		...
		default:
			...
			case AlfEdit:
				...
				case VK_BACK:
					...
					break;
				case VK_RETURN:
					// If item has an id, send the command
					//   (useful if the app wants a property dialog to appear)
					// If not, find first button and send that command
					//   (useful for applying list of values)
					if ((flags & AlfIdMask) > 0) {
						SendClickCommand(flags);
					} else {
						int item;
						int total = al->total;
						for (item = selected; item < total; item++) {
							int flags = al->al[item].flags;
							if ((flags & AlfTypeMask) == AlfSeparator)
								break;

							if ((flags & AlfTypeMask) == AlfButton) {
								if (!(flags & AlfDisabled))
									SendClickCommand(flags);
								break;
							}
						}
	...
	case LB_SETSEL:
		...
		lParam = IdToItem(lParam ^ AttribListById);
		if (lParam == LB_ERR)
			return LB_ERR;

		al->al[lParam].flags = (al->al[lParam].flags & ~AlfCheckedMask) | wParam << AlfCheckedRs | AlfRedraw;
	...
	case LB_GETSEL:
		wParam = IdToItem(wParam ^ AttribListById);
		if (wParam == LB_ERR)
			return LB_ERR;
		return al->al[wParam].flags >> AlfCheckedRs;
...
LRESULT __stdcall AtrOverlayProc(HWND hwnd, UINT message, long wParam, long lParam)
{
	...
	case WM_PAINT:
	{
		LPTSTR text = (LPTSTR)GetWindowLong(hwnd, GWL_USERDATA);
		PAINTSTRUCT ps;
		...
		ExtTextOut(ps.hdc, 0,CaretY, ETO_OPAQUE, &rect, text,lstrlen(text), NULL);
		EndPaint(hwnd, &ps);
		return 0;
	}
...
static int __stdcall ResizeOverlay()
{
	...
	//if (GetSelectedFlags() && GetFocus()==self && (text=al->al[selected].text)) {
	if (hedit != NULL
		&& GetSelectedFlags()
		&& GetFocus()==self
...
static void __stdcall SetCaretX(unsigned int newx)
{
	...
	if (CaretX > 0) {
		...
		if (unicode) {
			// NT only, does not work on 9x
			GetTextExtentExPoint(hdc, text,len+1, 0,NULL, &caret[1],&sz); //NT
...
static void __stdcall DeleteChar()
{
	...
	LPTSTR text = al->al[selected].text + CaretX;
	if (!text || !(flags & AlfSizeMask)) return; // error if text ptr NULL or no length
	//memmove(text, text+1, lstrlen(text));
	RtlMoveMemory( (char *)text, (const char *)(text+1), (lstrlen(text)+1) * sizeof(TCHAR));
	//al->al[selected].flags |= AlfRedraw;
	//PostRedraw();
	//PostRedrawOverlay();
	ResizeOverlay();
...
static void __stdcall InsertChar(unsigned int newchar)
{
	LPTSTR text = al->al[selected].text;
	unsigned int len;
	if (!text) return; // error if text ptr NULL

	len=lstrlen(text);

	if (CaretX > len) {CaretX=len;     return;}
	if (len+1 >= (flags >> AlfSizeRs)) return;
...
{
	if (flags & AlfIdMask) {
		if (flags & AlfOffsetId) {
			SendMessage(GetParent(self),
				WM_COMMAND,
				((flags & AlfIdMask) + (flags >> AlfCheckedRs)) |ALN_CLICKED,
				(LPARAM)self);
		} else {
			SendMessage(GetParent(self),
				WM_COMMAND,
...
static LPTSTR __stdcall GetChoiceText(TCHAR* text)
{
	if (text) {
		...
		if (value) {
			...
			case AlfToggle:
				do {
					text += lstrlen(text)+1; // skip ahead by size of string + NULL
...
static int AppendString(HGLOBAL* phmt, TCHAR* tail)
{
	TCHAR* text = *phmt; // get pointer to existing string
	unsigned int textlen, taillen, memsize;

	//debugwrite("-@%08X %d text=%S",text,tsize,text);

	if (!text || !tail) return 0; // null pointer so return 0 size
	...
	if ((textlen+taillen) * sizeof(TCHAR) >= memsize) { // reallocate if necessary
		text = GlobalHandle(text);
		GlobalUnlock( text ); // get handle from pointer and unlock (no effect if static)
		if (!(text = GlobalReAlloc(text,((textlen+taillen) * sizeof(TCHAR) )+256,GMEM_MOVEABLE)) )
			return textlen;
		*phmt = text = GlobalLock(text); // lock to get new pointer (no effect if static)
	}
...
HMENU AtrListToMenu(AttribList* al)
{
	...
	for (item = 0; item < al->total; item++) {
		int flags = al->al[item].flags;
		if (flags & AlfHidden) continue;
		switch (flags & AlfTypeMask) {
		case AlfTitle:
			if (hmChild != NULL) {
			...
			lstrcpy(&title[1], al->al[item].label);
			hmChild = CreateMenu();
			break;
		case AlfButton: // simple push button
			lstrcpy(&label[1], al->al[item].label);
			AppendMenu(hmChild, MF_STRING, flags & AlfIdMask, label);
			break;
		//case AlfToggle: // toggle button with checked/unchecked
			break;
		//case AlfMenu: // multichoice menu
			break;
		}
	}
	if (hmChild != NULL) {
