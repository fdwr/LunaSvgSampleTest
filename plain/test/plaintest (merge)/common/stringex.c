...
\param[in]	string	ASCIIZ string to parse words.
					Each word will be ended by a null (string is modified).
...
\warning	The string IS modified (nulls put at the end of each word).
\warning	If you are using the null pointer as a sentinel in the word array
			iteration, pass wordsmax+1 to guarantee the last enty is null.
			Otherwise, always use the returned word count, and do not exceed
...
\param[in]	strmax	size of buffer in characters (not bytes).
