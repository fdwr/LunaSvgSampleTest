...
\remark	The control eats the Enter keypress when a button is selected.
		It also does so if the LBS_NOTIFY style is set, notifying the
		parent window with the appropriate command event.
*/
...
typedef enum
{
	...
	AlfOffsetId=1<<23, // offset the id by toggle/menu value (so id+0 or id+1)
	...
	AlfSizeRs=24, //for text edit, tells size of buffer (so maximum num chars is really size-1). DO NOT use values > 255.
	AlfSizeMask=-1<<24,
...
#ifndef ATRBLIST_C
extern "C" WNDCLASS wcAtrList;
extern "C" WNDCLASS wcAtrOverlay;
extern "C" HMENU AtrListToMenu(AttribList* al);
#endif
